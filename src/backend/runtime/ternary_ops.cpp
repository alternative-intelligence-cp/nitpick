...
int16_t npk_trit_add_carry(int8_t a, int8_t b, int8_t carry_in) {
...
int8_t npk_trit_add(int8_t a, int8_t b) {
...
int8_t npk_trit_sub(int8_t a, int8_t b) {
    // ERR propagation (sticky error)
    if (a == -128 || b == -128) return -128;  // TRIT_ERR
    
    return npk_trit_add(a, static_cast<int8_t>(-b));
}

int8_t npk_trit_mul(int8_t a, int8_t b) {
    // ERR propagation (sticky error)
    if (a == -128 || b == -128) return -128;  // TRIT_ERR
...
int8_t npk_trit_or(int8_t a, int8_t b) {
...
uint16_t npk_tryte_add(uint16_t a, uint16_t b) {
    // Lazy initialization
    ...
    for (int i = 0; i < 5; ++i) {
        int16_t result = npk_trit_add_carry(ta_lo.trits[i], tb_lo.trits[i], carry);
    ...
    for (int i = 0; i < 5; ++i) {
        int16_t result = npk_trit_add_carry(ta_hi.trits[i], tb_hi.trits[i], carry);
        carry = static_cast<int8_t>(result >> 8);
...
uint16_t npk_tryte_sub(uint16_t a, uint16_t b) {
    // Subtraction via negation: a - b = a + (-b)
    uint16_t neg_b = npk_tryte_neg(b);
    return npk_tryte_add(a, neg_b);
}

uint16_t npk_tryte_neg(uint16_t a) {
    if (!g_initialized) init_lut();
...
uint16_t npk_tryte_mul(uint16_t a, uint16_t b) {
    // For multiplication, convert to binary, multiply, convert back
    if (a == TRYTE_ERR || b == TRYTE_ERR) return TRYTE_ERR;
    
    int32_t a_bin = npk_tryte_to_bin(a);
    int32_t b_bin = npk_tryte_to_bin(b);
    ...
    return npk_bin_to_tryte(static_cast<int32_t>(product));
}

uint16_t npk_tryte_div(uint16_t a, uint16_t b) {
    if (a == TRYTE_ERR || b == TRYTE_ERR) return TRYTE_ERR;
    ...
    return npk_bin_to_tryte(quotient);
}

uint16_t npk_tryte_mod(uint16_t a, uint16_t b) {
    // Sticky Error Propagation
    if (a == TRYTE_ERR || b == TRYTE_ERR) return TRYTE_ERR;
    
    // Unbias (convert to semantic values)
    int32_t val_a = npk_tryte_to_bin(a);
    int32_t val_b = npk_tryte_to_bin(b);
    ...
    // Re-bias and return
    return npk_bin_to_tryte(result_balanced);
}

int32_t npk_tryte_cmp(uint16_t a, uint16_t b) {
...
uint16_t npk_bin_to_tryte(int32_t value) {
    if (!g_initialized) init_lut();
...
int32_t npk_tryte_to_bin(uint16_t tryte) {
    if (!g_initialized) init_lut();
...
int8_t npk_tryte_get_trit(uint16_t tryte, uint8_t index) {
    if (!g_initialized) init_lut();
...
int8_t npk_nit_add(int8_t a, int8_t b) {
    // Sticky error propagation
...
int8_t npk_nit_sub(int8_t a, int8_t b) {
    // Sticky error propagation
    if (a == NIT_ERR || b == NIT_ERR) return NIT_ERR;
    
    return npk_nit_add(a, static_cast<int8_t>(-b));
}

int8_t npk_nit_mul(int8_t a, int8_t b) {
    // Sticky error propagation
    if (a == NIT_ERR || b == NIT_ERR) return NIT_ERR;
...
int8_t npk_nit_and(int8_t a, int8_t b) {
    // Kleene AND: min(a, b)
    if (a == NIT_ERR || b == NIT_ERR) return NIT_ERR;
    return (a < b) ? a : b;
}

int8_t npk_nit_or(int8_t a, int8_t b) {
    // Kleene OR: max(a, b)
    if (a == NIT_ERR || b == NIT_ERR) return NIT_ERR;
    return (a > b) ? a : b;
}

int8_t npk_nit_not(int8_t a) {
    // NOT: logical negation
    if (a == NIT_ERR) return NIT_ERR;
    return static_cast<int8_t>(-a);
}

int8_t npk_nit_is_err(int8_t a) {
...
uint16_t npk_nyte_add(uint16_t a, uint16_t b) {
...
uint16_t npk_nyte_sub(uint16_t a, uint16_t b) {
    if (a == NYTE_DIRECT_ERR || b == NYTE_DIRECT_ERR) return NYTE_DIRECT_ERR;
    int32_t a_val = static_cast<int32_t>(static_cast<int16_t>(a));
    int32_t b_val = static_cast<int32_t>(static_cast<int16_t>(b));
...
uint16_t npk_nyte_mul(uint16_t a, uint16_t b) {
    if (a == NYTE_DIRECT_ERR || b == NYTE_DIRECT_ERR) return NYTE_DIRECT_ERR;
    int32_t a_val = static_cast<int32_t>(static_cast<int16_t>(a));
    int32_t b_val = static_cast<int32_t>(static_cast<int16_t>(b));
...
uint16_t npk_nyte_div(uint16_t a, uint16_t b) {
    if (a == NYTE_DIRECT_ERR || b == NYTE_DIRECT_ERR) return NYTE_DIRECT_ERR;
    int32_t a_val = static_cast<int32_t>(static_cast<int16_t>(a));
    int32_t b_val = static_cast<int32_t>(static_cast<int16_t>(b));
...
uint16_t npk_nyte_mod(uint16_t a, uint16_t b) {
    if (a == NYTE_DIRECT_ERR || b == NYTE_DIRECT_ERR) return NYTE_DIRECT_ERR;
...
int8_t npk_nyte_get_nit(uint16_t nyte, uint8_t index) {
 ...
 */
extern "C" uint16_t npk_pack_nyte(int8_t n0, int8_t n1, int8_t n2, int8_t n3, int8_t n4) {
 ...
 */
extern "C" void npk_unpack_nyte(uint16_t packed, int8_t* n0, int8_t* n1, int8_t* n2, int8_t* n3, int8_t* n4) {
 ...
 */
extern "C" AriaWave9 npk_wave_lerp_dims(
 ...
 */
extern "C" int8_t npk_lerp_component(int32_t a, int32_t b, int32_t t) {
