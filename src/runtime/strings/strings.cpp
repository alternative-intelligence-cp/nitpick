...
static AriaResultPtr alloc_string_result(const char* data, int64_t length) {
    AriaString* str = (AriaString*)npk_gc_alloc(sizeof(AriaString), 0);
    if (!str) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
            "Failed to allocate string structure",
            __FILE__, __LINE__
        );
        return npk_result_err_ptr(error);
    }
    str->data = data;
    str->length = length;
    return npk_result_ok_ptr(str);
}
...
AriaResultPtr npk_string_from_cstr(const char* cstr) {
    if (!cstr) {
        AriaError* error = npk_error_new(
            ARIA_ERR_NULL_PTR,
    ...
    return npk_string_from_bytes(cstr, length);
}

AriaResultPtr npk_string_from_bytes(const char* data, int64_t length) {
    if (!data && length > 0) {
        AriaError* error = npk_error_new(
            ARIA_ERR_NULL_PTR,
    ...
    char* copied_data = (char*)npk_gc_alloc(length + 1, 0);
    if (!copied_data) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
...
int64_t npk_string_length(AriaString str) {
    return str.length;
}

bool npk_string_is_empty(AriaString str) {
    return str.length == 0;
}

bool npk_string_equals(AriaString a, AriaString b) {
...
int32_t npk_string_compare_str(AriaString a, AriaString b) {
...
AriaResultPtr npk_string_substring(AriaString str, int64_t start, int64_t end) {
    // Bounds checking
    ...
    // Empty substring
    if (sub_length == 0) {
        return npk_result_ok_ptr(npk_string_empty());
    }
    
    // Create new string from substring
    return npk_string_from_bytes(str.data + start, sub_length);
}

AriaResultPtr npk_string_from_char(uint8_t ch) {
    // Create a single-character string
    char* char_data = (char*)npk_gc_alloc(2, 0);  // 1 byte + null terminator
    if (!char_data) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
...
AriaResultI64 npk_string_index_of(AriaString haystack, AriaString needle) {
    if (needle.length == 0) {
        // Empty needle always matches at position 0
        return npk_result_ok_i64(0);
    }
    
    if (needle.length > haystack.length) {
        // Needle longer than haystack, cannot match
        return npk_result_ok_i64(-1);
    }
...
bool npk_string_contains(AriaString haystack, AriaString needle) {
    AriaResultI64 result = npk_string_index_of(haystack, needle);
    return result.value >= 0;
}

bool npk_string_starts_with(AriaString str, AriaString prefix) {
...
bool npk_string_ends_with(AriaString str, AriaString suffix) {
...
AriaResultPtr npk_string_trim(AriaString str) {
    if (str.length == 0) {
        { AriaString* heap_str = (AriaString*)npk_gc_alloc(sizeof(AriaString), 0); if (!heap_str) return npk_result_err_ptr(npk_error_new(ARIA_ERR_OUT_OF_MEMORY, "Failed to allocate string", __FILE__, __LINE__)); *heap_str = str; return npk_result_ok_ptr(heap_str); }
    }
    ...
    return npk_string_substring(str, start, end + 1);
}

AriaResultPtr npk_string_trim_start(AriaString str) {
    if (str.length == 0) {
        { AriaString* heap_str = (AriaString*)npk_gc_alloc(sizeof(AriaString), 0); if (!heap_str) return npk_result_err_ptr(npk_error_new(ARIA_ERR_OUT_OF_MEMORY, "Failed to allocate string", __FILE__, __LINE__)); *heap_str = str; return npk_result_ok_ptr(heap_str); }
    }
    ...
    return npk_string_substring(str, start, str.length);
}

AriaResultPtr npk_string_trim_end(AriaString str) {
    if (str.length == 0) {
        { AriaString* heap_str = (AriaString*)npk_gc_alloc(sizeof(AriaString), 0); if (!heap_str) return npk_result_err_ptr(npk_error_new(ARIA_ERR_OUT_OF_MEMORY, "Failed to allocate string", __FILE__, __LINE__)); *heap_str = str; return npk_result_ok_ptr(heap_str); }
    }
    ...
    return npk_string_substring(str, 0, end + 1);
}

AriaResultPtr npk_string_to_upper(AriaString str) {
    if (str.length == 0) {
        { AriaString* heap_str = (AriaString*)npk_gc_alloc(sizeof(AriaString), 0); if (!heap_str) return npk_result_err_ptr(npk_error_new(ARIA_ERR_OUT_OF_MEMORY, "Failed to allocate string", __FILE__, __LINE__)); *heap_str = str; return npk_result_ok_ptr(heap_str); }
    }
    
    // Allocate new string data
    char* upper_data = (char*)npk_gc_alloc(str.length + 1, 0);
    if (!upper_data) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
    ...
    { AriaString* heap_str = (AriaString*)npk_gc_alloc(sizeof(AriaString), 0); if (!heap_str) return npk_result_err_ptr(npk_error_new(ARIA_ERR_OUT_OF_MEMORY, "Failed to allocate string", __FILE__, __LINE__)); *heap_str = result; return npk_result_ok_ptr(heap_str); }
}

AriaResultPtr npk_string_to_lower(AriaString str) {
    if (str.length == 0) {
        { AriaString* heap_str = (AriaString*)npk_gc_alloc(sizeof(AriaString), 0); if (!heap_str) return npk_result_err_ptr(npk_error_new(ARIA_ERR_OUT_OF_MEMORY, "Failed to allocate string", __FILE__, __LINE__)); *heap_str = str; return npk_result_ok_ptr(heap_str); }
    }
    
    // Allocate new string data
    char* lower_data = (char*)npk_gc_alloc(str.length + 1, 0);
    if (!lower_data) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
...
AriaResultPtr npk_string_concat(AriaString a, AriaString b) {
    ...
    // Allocate new string data
    char* concat_data = (char*)npk_gc_alloc(total_length + 1, 0);
    if (!concat_data) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
...
AriaResultPtr npk_string_concat_n(AriaString* strings, int64_t count) {
    ...
    if (count == 1) {
        ...
        return npk_string_from_bytes(strings[0].data, strings[0].length);
    }
    ...
    AriaString result = {concat_data, total_length};
    AriaString* heap_str = (AriaString*)npk_gc_alloc(sizeof(AriaString), 0);
    if (!heap_str) {
        return npk_result_err_ptr(npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
            "Failed to allocate string structure",
            __FILE__, __LINE__
        ));
    ...
    *heap_str = result;
    return npk_result_ok_ptr(heap_str);
}

AriaString* npk_string_concat_n_simple(AriaString* strings, int64_t count) {
    AriaResultPtr result = npk_string_concat_n(strings, count);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

AriaResultPtr npk_string_repeat(AriaString str, int64_t count) {
    if (count < 0) {
        AriaError* error = npk_error_new(
            ARIA_ERR_INVALID_ARG,
    ...
    // Allocate new string data
    char* repeat_data = (char*)npk_gc_alloc(total_length + 1, 0);
    if (!repeat_data) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
...
AriaResultPtr npk_string_split(AriaString str, AriaString delimiter) {
    // Empty string returns empty array
    if (str.length == 0) {
        return npk_array_new(sizeof(AriaString*), 0, 0);
    }
    
    // Empty delimiter: split into individual bytes
    if (delimiter.length == 0) {
        AriaResultPtr array_result = npk_array_new(sizeof(AriaString*), str.length, 0);
        if (array_result.is_error) {
            return npk_result_err_ptr((AriaError*)array_result.error);
        }
        
        AriaArray* array = (AriaArray*)array_result.value;
        for (int64_t i = 0; i < str.length; i++) {
            AriaResultPtr char_str = npk_string_from_bytes(str.data + i, 1);
            if (char_str.is_error) {
                return npk_result_err_ptr((AriaError*)char_str.error);
            }
            
            AriaString* str_ptr = (AriaString*)char_str.value;
            AriaResultVoid push_result = npk_array_push(array, &str_ptr);
            if (push_result.is_error) {
                return npk_result_err_ptr((AriaError*)push_result.error);
            }
        }
        
        return npk_result_ok_ptr(array);
    }
    ...
    // Create array
    AriaResultPtr array_result = npk_array_new(sizeof(AriaString*), count, 0);
    if (array_result.is_error) {
        return npk_result_err_ptr((AriaError*)array_result.error);
    }
    ...
    for (int64_t i = 0; i <= str.length - delimiter.length; i++) {
        if (memcmp(str.data + i, delimiter.data, delimiter.length) == 0) {
            ...
            AriaResultPtr part = npk_string_from_bytes(str.data + start, i - start);
            if (part.is_error) {
                return npk_result_err_ptr((AriaError*)part.error);
            }
            
            AriaString* part_ptr = (AriaString*)part.value;
            AriaResultVoid push_result = npk_array_push(array, &part_ptr);
            if (push_result.is_error) {
                return npk_result_err_ptr((AriaError*)push_result.error);
            }
    ...
    // Add final part
    AriaResultPtr final_part = npk_string_from_bytes(str.data + start, str.length - start);
    if (final_part.is_error) {
        return npk_result_err_ptr((AriaError*)final_part.error);
    }
    
    AriaString* final_ptr = (AriaString*)final_part.value;
    AriaResultVoid push_result = npk_array_push(array, &final_ptr);
    if (push_result.is_error) {
        return npk_result_err_ptr((AriaError*)push_result.error);
    }
...
AriaResultPtr npk_string_join(const AriaArray* strings, AriaString delimiter) {
    ...
    char* joined_data = (char*)npk_gc_alloc(total_length + 1, 0);
    if (!joined_data) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
...
AriaResultPtr npk_string_to_cstr(AriaString str) {
    // String data is already null-terminated in npk_string_from_bytes
    // Just return the data pointer
    return npk_result_ok_ptr((void*)str.data);
}
...
AriaResultPtr npk_string_from_int(int64_t value) {
    ...
    return npk_string_from_bytes(buffer, len);
}

AriaResultI64 npk_string_to_int(AriaString str) {
    if (str.length == 0) {
        AriaError* error = npk_error_new(
            ARIA_ERR_INVALID_ARG,
        ...
        return npk_result_err_i64(error);
    }
    ...
    return npk_result_ok_i64(result);
}

AriaResultPtr npk_string_to_hex(AriaString str) {
    if (str.length == 0) {
        return npk_result_ok_ptr(npk_string_empty());
    }
    ...
    char* hex_data = (char*)npk_gc_alloc(hex_len + 1, 0);
    if (!hex_data) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
    ...
    AriaString* heap_str = (AriaString*)npk_gc_alloc(sizeof(AriaString), 0);
    if (!heap_str) {
        return npk_result_err_ptr(npk_error_new(ARIA_ERR_OUT_OF_MEMORY, "Failed to allocate string", __FILE__, __LINE__));
    }
    *heap_str = result;
    return npk_result_ok_ptr(heap_str);
}

AriaResultPtr npk_string_pad_right(AriaString str, int64_t total_length, uint8_t pad_char) {
    if (total_length <= str.length) {
        // Already long enough, return copy
        return npk_string_from_bytes(str.data, str.length);
    }

    int64_t padding_needed = total_length - str.length;
    char* padded_data = (char*)npk_gc_alloc(total_length + 1, 0);
    if (!padded_data) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
...
AriaResultPtr npk_string_format_float(double value, int32_t precision) {
...
AriaString* npk_string_from_int_simple(int64_t value) {
    AriaResultPtr result = npk_string_from_int(value);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

int64_t npk_string_to_int_simple(AriaString* str) {
    AriaResultI64 result = npk_string_to_int(*str);
    if (result.error != NULL) { std::abort(); }
    return result.value;
}

AriaString* npk_string_to_hex_simple(AriaString* str) {
    AriaResultPtr result = npk_string_to_hex(*str);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

AriaString* npk_string_pad_right_simple(AriaString* str, int64_t total_length, uint8_t pad_char) {
    AriaResultPtr result = npk_string_pad_right(*str, total_length, pad_char);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

AriaString* npk_string_format_float_simple(double value, int32_t precision) {
    AriaResultPtr result = npk_string_format_float(value, precision);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

AriaString* npk_string_from_char_simple(uint8_t ch) {
    AriaResultPtr result = npk_string_from_char(ch);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

AriaString* npk_string_from_cstr_simple(const char* cstr) {
    AriaResultPtr result = npk_string_from_cstr(cstr);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}
...
extern "C" AriaString* npk_sort_lines(AriaString* content) {
    if (!content || content->length == 0) {
        return npk_string_from_cstr_simple("");
    }
    ...
    return npk_string_from_cstr_simple(result.c_str());
}

AriaString* npk_string_concat_simple(AriaString* a, AriaString* b) {
    AriaResultPtr result = npk_string_concat(*a, *b);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}
...
AriaString* npk_string_repeat_simple(AriaString* str, int64_t count) {
    AriaResultPtr result = npk_string_repeat(*str, count);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

AriaString* npk_string_trim_start_simple(AriaString* str) {
    AriaResultPtr result = npk_string_trim_start(*str);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

AriaString* npk_string_trim_end_simple(AriaString* str) {
    AriaResultPtr result = npk_string_trim_end(*str);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

AriaString* npk_string_trim_simple(AriaString* str) {
    AriaResultPtr result = npk_string_trim(*str);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

AriaString* npk_string_to_upper_simple(AriaString* str) {
    AriaResultPtr result = npk_string_to_upper(*str);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

AriaString* npk_string_to_lower_simple(AriaString* str) {
    AriaResultPtr result = npk_string_to_lower(*str);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

int64_t npk_string_index_of_simple(AriaString* haystack, AriaString* needle) {
    AriaResultI64 result = npk_string_index_of(*haystack, *needle);
    if (result.error != NULL) { std::abort(); }
    return result.value;
}

AriaString* npk_string_substring_simple(AriaString* str, int64_t start, int64_t end) {
    AriaResultPtr result = npk_string_substring(*str, start, end);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}

// pad_left: prepend pad_char until string reaches total_length
AriaResultPtr npk_string_pad_left(AriaString str, int64_t total_length, uint8_t pad_char) {
    ...
    char* buf = (char*)npk_gc_alloc(full_len + 1, 0);
    if (!buf) {
        return npk_result_err_ptr(npk_error_new(
...
AriaString* npk_string_pad_left_simple(AriaString* str, int64_t total_length, uint8_t pad_char) {
    AriaResultPtr result = npk_string_pad_left(*str, total_length, pad_char);
    if (result.is_error) { std::abort(); }
    return (AriaString*)result.value;
}
...
// Example: 255 → "ff",  0 → "0"
AriaString* npk_string_from_int_hex_simple(int64_t value) {
    ...
    char* data = (char*)npk_gc_alloc(len + 1, 0);
    if (!data) std::abort();
    memcpy(data, buf, len);
    data[len] = '\0';
    AriaString* result = (AriaString*)npk_gc_alloc(sizeof(AriaString), 0);
    if (!result) std::abort();
    result->data = data;
    result->length = len;
    return result;
...
// Extern declaration in Aria: extern func:npk_string_byte_at = int64(string:s, int64:i);
...
int64_t npk_string_byte_at(const char* data, int64_t idx) {
    if (!data || idx < 0) return -1;
    return (int64_t)(unsigned char)data[idx];
}

// ── Conversion helpers (v0.3.0 — used by aria-libc) ──────────────────────────
// Use "npk_conv_" prefix to avoid collisions with existing AriaResultPtr variants.

extern "C" {

// string → float (strtod wrapper)
// extern func:npk_conv_to_float = flt64(string:s);
double npk_conv_to_float(const char* s) {
    if (!s) return 0.0;
    char *endptr = NULL;
...
// extern func:npk_conv_from_float = string(flt64:val);
AriaString npk_conv_from_float(double val) {
    char buf[64];
...
// int → string
// extern func:npk_conv_from_int = string(int64:val);
AriaString npk_conv_from_int(int64_t val) {
    char buf[32];
