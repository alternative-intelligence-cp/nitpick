...
uint64_t npk_hash_string(const char* str) {
    if (!str) return 0;
    return npk_hash_bytes(str, strlen(str));
}
...
static AriaResultVoid resize_map(AriaMap* map, size_t new_capacity) {
    ...
    if (!new_entries) {
        return npk_result_err_void(npk_error_msg("Map resize failed: out of memory"));
    }
...
AriaResultPtr npk_map_new(size_t key_size, size_t value_size, size_t initial_capacity) {
    if (key_size == 0 || value_size == 0) {
        return npk_result_err_ptr(npk_error_msg("Map creation failed: key_size and value_size must be > 0"));
    }
    
    // Allocate map structure
    AriaMap* map = (AriaMap*)malloc(sizeof(AriaMap));
    if (!map) {
        return npk_result_err_ptr(npk_error_msg("Map creation failed: out of memory"));
    }
...
AriaResultVoid npk_map_insert(AriaMap* map, const void* key, const void* value) {
    if (!map || !key || !value) {
        return npk_result_err_void(npk_error_msg("Map insert failed: NULL argument"));
    }
    ...
    // Hash the key
    uint64_t hash = npk_hash_bytes(key, map->key_size);
    ...
    if (!map->entries[index].key || !map->entries[index].value) {
        ...
        return npk_result_err_void(npk_error_msg("Map insert failed: out of memory"));
    }
...
AriaResultPtr npk_map_get(const AriaMap* map, const void* key) {
    if (!map || !key) {
        return npk_result_err_ptr(npk_error_msg("Map get failed: NULL argument"));
    }
    ...
    // Check if key exists
    if (map->entries[index].occupied) {
        return npk_result_ok_ptr(map->entries[index].value);
    }
    
    return npk_result_err_ptr(npk_error_msg("Map get failed: key not found"));
}

AriaResultVoid npk_map_remove(AriaMap* map, const void* key) {
    if (!map || !key) {
        return npk_result_err_void(npk_error_msg("Map remove failed: NULL argument"));
    }
    ...
    // Check if key exists
    if (!map->entries[index].occupied) {
        return npk_result_err_void(npk_error_msg("Map remove failed: key not found"));
    }
...
AriaMap* npk_map_new_simple(size_t key_size, size_t value_size) {
    AriaResultPtr result = npk_map_new(key_size, value_size, 0);
    if (result.error) {
...
void npk_map_insert_simple(AriaMap* map, const void* key, const void* value) {
    AriaResultVoid result = npk_map_insert(map, key, value);
    if (result.error) {
...
void* npk_map_get_simple(AriaMap* map, const void* key) {
    AriaResultPtr result = npk_map_get(map, key);
    if (result.error) {
