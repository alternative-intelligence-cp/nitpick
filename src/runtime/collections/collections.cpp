...
AriaResultPtr npk_array_new(size_t element_size, size_t initial_capacity, int type_id) {
    ...
    AriaArray* array = (AriaArray*)npk_gc_alloc(sizeof(AriaArray), 0);
    if (!array) {
        AriaError* error = npk_error_new(
    ...
    array->data = npk_gc_alloc(element_size * capacity, type_id);
    if (!array->data) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
...
AriaResultPtr npk_array_get(const AriaArray* array, size_t index) {
    ...
    void* element = npk_array_get_unchecked(array, index);
    return npk_result_ok_ptr(element);
}
...
AriaResultVoid npk_array_set(AriaArray* array, size_t index, const void* value) {
    if (!array) {
        ...
        return npk_result_err_void(error);
    }
    ...
    npk_array_set_unchecked(array, index, value);
...
AriaResultVoid npk_array_push(AriaArray* array, const void* value) {
    ...
    if (array->length >= array->capacity) {
        ...
        void* new_data = npk_gc_alloc(array->element_size * new_capacity, array->type_id);
        if (!new_data) {
            AriaError* error = npk_error_new(
                ARIA_ERR_OUT_OF_MEMORY,
    ...
    // Copy element to end
    npk_array_set_unchecked(array, array->length, value);
    array->length++;
...
AriaResultVoid npk_array_pop(AriaArray* array, void* out_value) {
    ...
    if (out_value) {
        void* element = npk_array_get_unchecked(array, array->length - 1);
...
AriaResultPtr npk_array_slice(const AriaArray* array, size_t start, size_t end) {
    ...
    AriaResultPtr result = npk_array_new(array->element_size, slice_length, array->type_id);
    ...
    for (size_t i = 0; i < slice_length; i++) {
        void* src = npk_array_get_unchecked(array, start + i);
        npk_array_set_unchecked(slice, i, src);
    }
    slice->length = slice_length;
    
    return npk_result_ok_ptr(slice);
}
...
AriaResultPtr npk_array_filter(const AriaArray* array, AriaPredicateFn predicate, void* context) {
    ...
    AriaResultPtr result = npk_array_new(array->element_size, array->capacity, array->type_id);
    if (result.is_error) return result;
    ...
    for (size_t i = 0; i < array->length; i++) {
        void* element = npk_array_get_unchecked(array, i);
        if (predicate(element, i, context)) {
            AriaResultVoid push_result = npk_array_push(filtered, element);
    ...
    return npk_result_ok_ptr(filtered);
}
...
AriaResultPtr npk_array_transform(const AriaArray* array, AriaMapperFn mapper,
    ...
    AriaResultPtr result = npk_array_new(output_element_size, array->length, output_type_id);
    if (result.is_error) return result;
    ...
    // Transform elements
    for (size_t i = 0; i < array->length; i++) {
        void* in_element = npk_array_get_unchecked(array, i);
        void* out_element = npk_array_get_unchecked(transformed, i);
        mapper(in_element, i, out_element, context);
    }
    
    return npk_result_ok_ptr(transformed);
}
...
AriaResultPtr npk_array_reduce(const AriaArray* array, AriaReducerFn reducer,
    ...
    void* accumulator = npk_gc_alloc(accumulator_size, 0);
    if (!accumulator) {
        AriaError* error = npk_error_new(
            ARIA_ERR_OUT_OF_MEMORY,
    ...
    return npk_result_ok_ptr(accumulator);
}
...
AriaResultVoid npk_array_reverse(AriaArray* array) {
    ...
    while (left < right) {
        void* left_elem = npk_array_get_unchecked(array, left);
        void* right_elem = npk_array_get_unchecked(array, right);
...
AriaResultPtr npk_array_unique(const AriaArray* array, AriaComparatorFn comparator, void* context) {
    ...
    for (size_t i = 0; i < array->length; i++) {
        ...
        for (size_t j = 0; j < unique->length; j++) {
            void* existing = npk_array_get_unchecked(unique, j);
        ...
        if (!found) {
            AriaResultVoid push_result = npk_array_push(unique, element);
            if (push_result.is_error) {
                return npk_result_err_ptr((AriaError*)push_result.error);
            }
        }
    ...
    return npk_result_ok_ptr(unique);
}
...
AriaArray* npk_array_new_simple(size_t element_size, int type_id) {
    AriaResultPtr result = npk_array_new(element_size, ARIA_ARRAY_DEFAULT_CAPACITY, type_id);
    if (result.is_error) {
...
void npk_array_push_simple(AriaArray* array, const void* value) {
    AriaResultVoid result = npk_array_push(array, value);
    if (result.is_error) {
        std::abort();
    }
}
...
void* npk_array_get_simple(AriaArray* array, size_t index) {
    AriaResultPtr result = npk_array_get(array, index);
    if (result.is_error) {
        std::abort();
    }
    return result.value;
...
void npk_array_set_simple(AriaArray* array, size_t index, const void* value) {
    AriaResultVoid result = npk_array_set(array, index, value);
    if (result.is_error) {
        std::abort();
    }
}
...
void npk_array_pop_simple(AriaArray* array, void* out_value) {
    AriaResultVoid result = npk_array_pop(array, out_value);
    if (result.is_error) {
        std::abort();
    }
}
