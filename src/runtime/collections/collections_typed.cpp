...
extern "C" {
extern void* npk_array_new_simple(size_t element_size, int type_id);
extern void  npk_array_push_simple(void* arr, const void* val);
extern void  npk_array_pop_simple(void* arr, void* out_val);
extern void* npk_array_get_simple(void* arr, size_t index);
extern void  npk_array_set_simple(void* arr, size_t index, const void* val);
extern size_t npk_array_length(const void* arr);
extern void  npk_array_free(void* arr);

extern void* npk_map_new_simple(size_t key_size, size_t value_size);
extern void  npk_map_insert_simple(void* map, const void* key, const void* value);
extern void* npk_map_get_simple(void* map, const void* key);
extern int32_t npk_map_has(void* map, const void* key);
extern void  npk_map_remove(void* map, const void* key);
extern int64_t npk_map_length(const void* map);
extern void  npk_map_clear(void* map);
extern void  npk_map_free(void* map);
}
...
void* vec_i64_new(void) {
    return npk_array_new_simple(8, 1);
}

void vec_i64_push(void* arr, int64_t val) {
    npk_array_push_simple(arr, &val);
}

int64_t vec_i64_pop(void* arr) {
    int64_t v = 0;
    npk_array_pop_simple(arr, &v);
    return v;
}

int64_t vec_i64_get(void* arr, int64_t index) {
    void* ptr = npk_array_get_simple(arr, index);
    int64_t v;
...
void vec_i64_set(void* arr, int64_t index, int64_t val) {
    npk_array_set_simple(arr, index, &val);
}

int64_t vec_i64_length(void* arr) {
    return npk_array_length(arr);
}
...
void* vec_i32_new(void) {
    return npk_array_new_simple(4, 1);
}
...
int64_t map_i64_get(void* map, int64_t key) {
    void* ptr = npk_map_get_simple(map, &key);
    int64_t v;
    memcpy(&v, ptr, sizeof(int64_t));
    return v;
}
...
int32_t map_i64_has(void* map, int64_t key) {
    return npk_map_has(map, &key);
}

void map_i64_remove(void* map, int64_t key) {
    npk_map_remove(map, &key);
}

int64_t map_i64_length(void* map) {
    return npk_map_length(map);
}
...
int32_t map_i32_i64_remove(void* map, int32_t key) {
    if (!npk_map_has(map, &key)) return 0;
    npk_map_remove(map, &key);
    return 1;
}
...
void* graph_new(void) {
    ...
    g->adj_map = npk_map_new_simple(8, 8);
    g->node_list = npk_array_new_simple(8, 1);
    g->edge_count = 0;
    return g;
}
...
void graph_add_node(void* graph, int64_t node_id) {
    ...
    if (!npk_map_has(g->adj_map, &node_id)) {
        void* neighbors = npk_array_new_simple(8, 1);
        int64_t handle = reinterpret_cast<int64_t>(neighbors);
        npk_map_insert_simple(g->adj_map, &node_id, &handle);
        npk_array_push_simple(g->node_list, &node_id);
    }
}

int32_t graph_has_node(void* graph, int64_t node_id) {
    AriaGraph* g = static_cast<AriaGraph*>(graph);
    return npk_map_has(g->adj_map, &node_id);
}
...
void graph_add_edge(void* graph, int64_t from, int64_t to) {
    ...
    void* handle_ptr = npk_map_get_simple(g->adj_map, &from);
    int64_t handle_val;
    memcpy(&handle_val, handle_ptr, sizeof(int64_t));
    void* neighbors = reinterpret_cast<void*>(handle_val);
    npk_array_push_simple(neighbors, &to);
    g->edge_count++;
}

int32_t graph_has_edge(void* graph, int64_t from, int64_t to) {
    AriaGraph* g = static_cast<AriaGraph*>(graph);
    if (!npk_map_has(g->adj_map, &from)) return 0;
    void* handle_ptr = npk_map_get_simple(g->adj_map, &from);
    int64_t handle_val;
    memcpy(&handle_val, handle_ptr, sizeof(int64_t));
    void* neighbors = reinterpret_cast<void*>(handle_val);
    int64_t len = static_cast<int64_t>(npk_array_length(neighbors));
    for (int64_t i = 0; i < len; i++) {
        void* elem_ptr = npk_array_get_simple(neighbors, i);
        int64_t elem;
...
int64_t graph_degree(void* graph, int64_t node_id) {
    AriaGraph* g = static_cast<AriaGraph*>(graph);
    if (!npk_map_has(g->adj_map, &node_id)) return 0;
    void* handle_ptr = npk_map_get_simple(g->adj_map, &node_id);
    int64_t handle_val;
    memcpy(&handle_val, handle_ptr, sizeof(int64_t));
    void* neighbors = reinterpret_cast<void*>(handle_val);
    return static_cast<int64_t>(npk_array_length(neighbors));
}
...
int64_t graph_neighbor_at(void* graph, int64_t node_id, int64_t index) {
    ...
    memcpy(&handle_val, handle_ptr, sizeof(int64_t));
    void* neighbors = reinterpret_cast<void*>(handle_val);
    void* elem_ptr = npk_array_get_simple(neighbors, index);
    int64_t elem;
    memcpy(&elem, elem_ptr, sizeof(int64_t));
    return elem;
}
...
int64_t graph_node_count(void* graph) {
    AriaGraph* g = static_cast<AriaGraph*>(graph);
    return npk_map_length(g->adj_map);
}
...
void graph_remove_edge(void* graph, int64_t from, int64_t to) {
    AriaGraph* g = static_cast<AriaGraph*>(graph);
    if (!npk_map_has(g->adj_map, &from)) return;
    void* handle_ptr = npk_map_get_simple(g->adj_map, &from);
    int64_t handle_val;
    memcpy(&handle_val, handle_ptr, sizeof(int64_t));
    void* neighbors = reinterpret_cast<void*>(handle_val);
    ...
    for (int64_t i = 0; i < len; i++) {
        ...
        if (elem == to) {
            ...
            if (i < len - 1) {
                void* last_ptr = npk_array_get_simple(neighbors, len - 1);
                int64_t last_val;
                memcpy(&last_val, last_ptr, sizeof(int64_t));
                npk_array_set_simple(neighbors, i, &last_val);
            }
            int64_t dummy;
            npk_array_pop_simple(neighbors, &dummy);
            g->edge_count--;
            return;
        }
    }
...
void graph_free(void* graph) {
    ...
    size_t n = npk_array_length(g->node_list);
    for (size_t i = 0; i < n; i++) {
        void* nid_ptr = npk_array_get_simple(g->node_list, i);
        int64_t nid;
        memcpy(&nid, nid_ptr, sizeof(int64_t));
        if (npk_map_has(g->adj_map, &nid)) {
            void* handle_ptr = npk_map_get_simple(g->adj_map, &nid);
            int64_t handle_val;
            memcpy(&handle_val, handle_ptr, sizeof(int64_t));
            void* neighbors = reinterpret_cast<void*>(handle_val);
            npk_array_free(neighbors);
        }
    }
    npk_array_free(g->node_list);
    npk_map_free(g->adj_map);
    delete g;
}
