...
int64_t npk_arena_new_handle(int64_t capacity) {
    AriaArenaResult result = npk_arena_new(static_cast<size_t>(capacity));
...
int64_t npk_arena_alloc_handle(int64_t handle, int64_t size) {
    ...
    npk_arena* arena = reinterpret_cast<npk_arena*>(handle);
    // npk_arena_alloc requires size and alignment (use 8-byte alignment)
    AriaAllocResult result = npk_arena_alloc(arena, static_cast<size_t>(size), 8);
