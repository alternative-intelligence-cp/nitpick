 ...
 * All functions prefixed with npk_shim_* to distinguish from the
 * underlying runtime API (npk_thread_*, npk_mutex_*, etc.).
 */
...
// ============================================================================
// Thread Lifecycle — npk_shim_thread_*
// ============================================================================
 ...
 */
int64_t npk_shim_thread_spawn(void* func, int64_t arg) {
    if (!func) return -1;
    ...
    AriaResult* res = npk_thread_create(thread_trampoline, data);
    if (!res || res->err) {
        free(data);
        if (res) npk_result_free(res);
        return -1;
    }
 ...
 */
int32_t npk_shim_thread_detach(int64_t handle) {
    if (handle < 0 || handle >= MAX_THREADS || !g_threads[handle]) return -1;
    AriaThread* t = g_threads[handle];
    AriaResult* res = npk_thread_detach(t);
    g_threads[handle] = NULL;
    if (!res || res->err) {
        if (res) npk_result_free(res);
        return -1;
...
void npk_shim_thread_yield(void) {
    npk_thread_yield();
}

/** Sleep for given nanoseconds. */
void npk_shim_thread_sleep_ns(int64_t ns) {
    npk_thread_sleep_ns((uint64_t)ns);
}

/** Sleep for given milliseconds (convenience). */
void npk_shim_thread_sleep_ms(int64_t ms) {
    npk_thread_sleep_ns((uint64_t)ms * 1000000ULL);
}

/** Set current thread's name. */
void npk_shim_thread_set_name(const char* name) {
    npk_thread_set_name(name);
}

/** Get hardware concurrency (number of CPUs). */
int32_t npk_shim_thread_hardware_concurrency(void) {
    return (int32_t)npk_thread_hardware_concurrency();
}

/** Get current thread ID as int64. */
int64_t npk_shim_thread_current_id(void) {
    AriaThreadId tid = npk_thread_current_id();
    return (int64_t)tid.id;
}

// ============================================================================
// Mutex — npk_shim_mutex_*
// ============================================================================

/** Create a mutex. type: 0=normal, 1=recursive. Returns handle or -1. */
int64_t npk_shim_mutex_create(int32_t type) {
    AriaResult* res = npk_mutex_create((AriaMutexType)type);
    if (!res || res->err) {
        if (res) npk_result_free(res);
        return -1;
    }
...
int32_t npk_shim_mutex_lock(int64_t handle) {
    if (handle < 0 || handle >= MAX_MUTEXES || !g_mutexes[handle]) return -1;
    AriaResult* res = npk_mutex_lock(g_mutexes[handle]);
...
int32_t npk_shim_mutex_trylock(int64_t handle) {
    if (handle < 0 || handle >= MAX_MUTEXES || !g_mutexes[handle]) return -1;
    AriaResult* res = npk_mutex_trylock(g_mutexes[handle]);
    if (!res || res->err) {
        if (res) npk_result_free(res);
        return -1;
    }
...
int32_t npk_shim_mutex_unlock(int64_t handle) {
    if (handle < 0 || handle >= MAX_MUTEXES || !g_mutexes[handle]) return -1;
    AriaResult* res = npk_mutex_unlock(g_mutexes[handle]);
    int32_t ret = (!res || res->err) ? -1 : 0;
    if (res) npk_result_free(res);
    return ret;
}
...
int32_t npk_shim_mutex_destroy(int64_t handle) {
    if (handle < 0 || handle >= MAX_MUTEXES || !g_mutexes[handle]) return -1;
    AriaResult* res = npk_mutex_destroy(g_mutexes[handle]);
...
// ============================================================================
// Condition Variables — npk_shim_condvar_*
// ============================================================================

/** Create a condition variable. Returns handle or -1. */
int64_t npk_shim_condvar_create(void) {
    AriaResult* res = npk_condvar_create();
    if (!res || res->err) {
        if (res) npk_result_free(res);
        return -1;
    }
...
int32_t npk_shim_condvar_wait(int64_t cv_handle, int64_t mutex_handle) {
    ...
    AriaResult* res = npk_condvar_wait(g_condvars[cv_handle], g_mutexes[mutex_handle]);
    int32_t ret = (!res || res->err) ? -1 : 0;
    if (res) npk_result_free(res);
    return ret;
}
...
int32_t npk_shim_condvar_timedwait(int64_t cv_handle, int64_t mutex_handle, int64_t timeout_ns) {
    if (cv_handle < 0 || cv_handle >= MAX_CONDVARS || !g_condvars[cv_handle]) return -1;
    if (mutex_handle < 0 || mutex_handle >= MAX_MUTEXES || !g_mutexes[mutex_handle]) return -1;
    AriaResult* res = npk_condvar_timedwait(g_condvars[cv_handle], g_mutexes[mutex_handle], (uint64_t)timeout_ns);
    if (!res || res->err) {
        if (res) npk_result_free(res);
        return -1;
    }
...
int32_t npk_shim_condvar_signal(int64_t handle) {
    if (handle < 0 || handle >= MAX_CONDVARS || !g_condvars[handle]) return -1;
    AriaResult* res = npk_condvar_signal(g_condvars[handle]);
    int32_t ret = (!res || res->err) ? -1 : 0;
    if (res) npk_result_free(res);
    return ret;
}
...
int32_t npk_shim_condvar_broadcast(int64_t handle) {
    if (handle < 0 || handle >= MAX_CONDVARS || !g_condvars[handle]) return -1;
    AriaResult* res = npk_condvar_broadcast(g_condvars[handle]);
    int32_t ret = (!res || res->err) ? -1 : 0;
    if (res) npk_result_free(res);
    return ret;
}
...
int32_t npk_shim_condvar_destroy(int64_t handle) {
    if (handle < 0 || handle >= MAX_CONDVARS || !g_condvars[handle]) return -1;
    AriaResult* res = npk_condvar_destroy(g_condvars[handle]);
...
// ============================================================================
// Read-Write Locks — npk_shim_rwlock_*
// ============================================================================

/** Create a read-write lock. Returns handle or -1. */
int64_t npk_shim_rwlock_create(void) {
    AriaResult* res = npk_rwlock_create();
    if (!res || res->err) {
        if (res) npk_result_free(res);
        return -1;
    }
...
int32_t npk_shim_rwlock_rdlock(int64_t handle) {
    if (handle < 0 || handle >= MAX_RWLOCKS || !g_rwlocks[handle]) return -1;
    AriaResult* res = npk_rwlock_rdlock(g_rwlocks[handle]);
    int32_t ret = (!res || res->err) ? -1 : 0;
    if (res) npk_result_free(res);
    return ret;
}

int32_t npk_shim_rwlock_tryrdlock(int64_t handle) {
    if (handle < 0 || handle >= MAX_RWLOCKS || !g_rwlocks[handle]) return -1;
    AriaResult* res = npk_rwlock_tryrdlock(g_rwlocks[handle]);
    if (!res || res->err) {
        if (res) npk_result_free(res);
        return -1;
    }
...
int32_t npk_shim_rwlock_wrlock(int64_t handle) {
    if (handle < 0 || handle >= MAX_RWLOCKS || !g_rwlocks[handle]) return -1;
    AriaResult* res = npk_rwlock_wrlock(g_rwlocks[handle]);
    int32_t ret = (!res || res->err) ? -1 : 0;
    if (res) npk_result_free(res);
    return ret;
}

int32_t npk_shim_rwlock_trywrlock(int64_t handle) {
    if (handle < 0 || handle >= MAX_RWLOCKS || !g_rwlocks[handle]) return -1;
    AriaResult* res = npk_rwlock_trywrlock(g_rwlocks[handle]);
    if (!res || res->err) {
        if (res) npk_result_free(res);
        return -1;
    }
...
int32_t npk_shim_rwlock_unlock(int64_t handle) {
    if (handle < 0 || handle >= MAX_RWLOCKS || !g_rwlocks[handle]) return -1;
    AriaResult* res = npk_rwlock_unlock(g_rwlocks[handle]);
    int32_t ret = (!res || res->err) ? -1 : 0;
    if (res) npk_result_free(res);
    return ret;
}

int32_t npk_shim_rwlock_destroy(int64_t handle) {
    if (handle < 0 || handle >= MAX_RWLOCKS || !g_rwlocks[handle]) return -1;
    AriaResult* res = npk_rwlock_destroy(g_rwlocks[handle]);
...
// ============================================================================
// Barriers — npk_shim_barrier_*
// ============================================================================

/** Create barrier for count threads. Returns handle or -1. */
int64_t npk_shim_barrier_create(int32_t count) {
    AriaResult* res = npk_barrier_create((uint32_t)count);
    if (!res || res->err) {
        if (res) npk_result_free(res);
        return -1;
    }
...
int32_t npk_shim_barrier_wait(int64_t handle) {
    if (handle < 0 || handle >= MAX_BARRIERS || !g_barriers[handle]) return -1;
    AriaResult* res = npk_barrier_wait(g_barriers[handle]);
    int32_t ret = (!res || res->err) ? -1 : 0;
    if (res) npk_result_free(res);
    return ret;
}

int32_t npk_shim_barrier_destroy(int64_t handle) {
    if (handle < 0 || handle >= MAX_BARRIERS || !g_barriers[handle]) return -1;
    AriaResult* res = npk_barrier_destroy(g_barriers[handle]);
...
// ============================================================================
// Thread-Local Storage — npk_shim_tls_*
// ============================================================================

/** Create TLS key. Returns handle or -1. */
int64_t npk_shim_tls_create(void) {
    AriaResult* res = npk_thread_local_create(NULL); // no destructor
    if (!res || res->err) {
        if (res) npk_result_free(res);
        return -1;
    }
...
int32_t npk_shim_tls_set(int64_t handle, int64_t value) {
    if (handle < 0 || handle >= MAX_TLS_KEYS || !g_tls_keys[handle]) return -1;
    AriaResult* res = npk_thread_local_set(g_tls_keys[handle], (void*)(uintptr_t)value);
    int32_t ret = (!res || res->err) ? -1 : 0;
    if (res) npk_result_free(res);
    return ret;
}
...
int64_t npk_shim_tls_get(int64_t handle) {
    if (handle < 0 || handle >= MAX_TLS_KEYS || !g_tls_keys[handle]) return 0;
    void* val = npk_thread_local_get(g_tls_keys[handle]);
    return (int64_t)(uintptr_t)val;
}

int32_t npk_shim_tls_destroy(int64_t handle) {
    if (handle < 0 || handle >= MAX_TLS_KEYS || !g_tls_keys[handle]) return -1;
    AriaResult* res = npk_thread_local_destroy(g_tls_keys[handle]);
...
int64_t npk_shim_channel_create(int32_t capacity) {
...
int64_t npk_shim_channel_create_unbuffered(void) {
    AriaChannel* ch = (AriaChannel*)calloc(1, sizeof(AriaChannel));
    if (!ch) return -1;
    ch->capacity = 0;
...
int64_t npk_shim_channel_create_oneshot(void) {
    AriaChannel* ch = (AriaChannel*)calloc(1, sizeof(AriaChannel));
    if (!ch) return -1;
    ch->capacity = 1;
...
int32_t npk_shim_channel_get_mode(int64_t handle) {
...
int32_t npk_shim_channel_try_send(int64_t handle, int64_t value) {
    if (handle < 0 || handle >= MAX_CHANNELS || !g_channels[handle]) return -1;
    AriaChannel* ch = g_channels[handle];
    pthread_mutex_lock(&ch->mtx);
 ...
 */
int64_t npk_shim_channel_recv(int64_t handle) {
...
int64_t npk_shim_channel_try_recv(int64_t handle) {
    if (handle < 0 || handle >= MAX_CHANNELS || !g_channels[handle]) return 0;
    AriaChannel* ch = g_channels[handle];
    pthread_mutex_lock(&ch->mtx);
...
int32_t npk_shim_channel_count(int64_t handle) {
    if (handle < 0 || handle >= MAX_CHANNELS || !g_channels[handle]) return 0;
    AriaChannel* ch = g_channels[handle];
    pthread_mutex_lock(&ch->mtx);
    int32_t c = ch->count;
...
int32_t npk_shim_channel_close(int64_t handle) {
    if (handle < 0 || handle >= MAX_CHANNELS || !g_channels[handle]) return -1;
    AriaChannel* ch = g_channels[handle];
    pthread_mutex_lock(&ch->mtx);
    ch->closed = true;
...
int32_t npk_shim_channel_destroy(int64_t handle) {
    if (handle < 0 || handle >= MAX_CHANNELS || !g_channels[handle]) return -1;
    AriaChannel* ch = g_channels[handle];
    npk_shim_channel_close(handle);
...
bool npk_shim_channel_is_closed(int64_t handle) {
 ...
 */
int32_t npk_shim_channel_select(int64_t* channels, int32_t n, int64_t timeout_ms) {
 ...
 */
int32_t npk_shim_channel_select2(int64_t ch0, int64_t ch1, int64_t timeout_ms) {
    int64_t chs[2] = { ch0, ch1 };
    return npk_shim_channel_select(chs, 2, timeout_ms);
}

/** select3: poll 3 channels. */
int32_t npk_shim_channel_select3(int64_t ch0, int64_t ch1, int64_t ch2, int64_t timeout_ms) {
    int64_t chs[3] = { ch0, ch1, ch2 };
    return npk_shim_channel_select(chs, 3, timeout_ms);
}

/** select4: poll 4 channels. */
int32_t npk_shim_channel_select4(int64_t ch0, int64_t ch1, int64_t ch2, int64_t ch3, int64_t timeout_ms) {
    int64_t chs[4] = { ch0, ch1, ch2, ch3 };
    return npk_shim_channel_select(chs, 4, timeout_ms);
}
...
static void actor_loop_wrapper(void* env, int64_t actor_handle) {
    ...
    while (1) {
        ...
        if (npk_shim_channel_is_closed(actor->mailbox_handle) &&
            npk_shim_channel_count(actor->mailbox_handle) == 0) {
            break;
        }

        // Receive message (blocks if empty, returns 0 if closed+empty)
        int64_t msg = npk_shim_channel_recv(actor->mailbox_handle);
        ...
        if (npk_shim_channel_is_closed(actor->mailbox_handle) &&
            npk_shim_channel_count(actor->mailbox_handle) == 0 && msg == 0) {
            break;
        }
 ...
 */
int64_t npk_shim_actor_spawn(void* behavior_func) {
    if (!behavior_func) return -1;

    // Create mailbox channel
    int64_t mailbox = npk_shim_channel_create((int32_t)ACTOR_MAILBOX_CAP);
    if (mailbox < 0) return -1;

    AriaActor* actor = (AriaActor*)calloc(1, sizeof(AriaActor));
    if (!actor) {
        npk_shim_channel_destroy(mailbox);
        return -1;
    }
    ...
    int64_t tid = npk_shim_thread_spawn((void*)actor_loop_wrapper, actor_handle);
    if (tid < 0) {
        npk_shim_channel_destroy(mailbox);
...
int32_t npk_shim_actor_send(int64_t actor_handle, int64_t message) {
    if (actor_handle < 0 || actor_handle >= MAX_ACTORS || !g_actors[actor_handle]) return -1;
    AriaActor* actor = g_actors[actor_handle];
    return npk_shim_channel_send(actor->mailbox_handle, message);
}

/** Try to send a message to actor without blocking. Returns 0 on success, -1 if full/stopped. */
int32_t npk_shim_actor_try_send(int64_t actor_handle, int64_t message) {
    if (actor_handle < 0 || actor_handle >= MAX_ACTORS || !g_actors[actor_handle]) return -1;
    AriaActor* actor = g_actors[actor_handle];
    return npk_shim_channel_try_send(actor->mailbox_handle, message);
}

/** Get the mailbox channel handle for an actor (for advanced channel ops). */
int64_t npk_shim_actor_mailbox(int64_t actor_handle) {
    if (actor_handle < 0 || actor_handle >= MAX_ACTORS || !g_actors[actor_handle]) return -1;
    return g_actors[actor_handle]->mailbox_handle;
}

/** Get pending message count in actor's mailbox. */
int32_t npk_shim_actor_pending(int64_t actor_handle) {
    if (actor_handle < 0 || actor_handle >= MAX_ACTORS || !g_actors[actor_handle]) return 0;
    return npk_shim_channel_count(g_actors[actor_handle]->mailbox_handle);
}

/** Check if actor is still running. Returns 1 if running, 0 if stopped/init, -1 on error. */
int32_t npk_shim_actor_is_alive(int64_t actor_handle) {
    if (actor_handle < 0 || actor_handle >= MAX_ACTORS || !g_actors[actor_handle]) return -1;
    AriaActor* actor = g_actors[actor_handle];
    pthread_mutex_lock(&actor->state_mtx);
...
int32_t npk_shim_actor_stop(int64_t actor_handle) {
    if (actor_handle < 0 || actor_handle >= MAX_ACTORS || !g_actors[actor_handle]) return -1;
    AriaActor* actor = g_actors[actor_handle];
    ...
    npk_shim_channel_close(actor->mailbox_handle);

    // Join actor thread
    int32_t ret = npk_shim_thread_join(actor->thread_handle);
    return ret;
}

/** Destroy an actor (stop + free resources). Returns 0 on success. */
int32_t npk_shim_actor_destroy(int64_t actor_handle) {
    if (actor_handle < 0 || actor_handle >= MAX_ACTORS || !g_actors[actor_handle]) return -1;
    AriaActor* actor = g_actors[actor_handle];
    ...
    if (running) {
        npk_shim_actor_stop(actor_handle);
    }

    // Destroy mailbox
    npk_shim_channel_destroy(actor->mailbox_handle);
