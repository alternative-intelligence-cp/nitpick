...
std::string Preprocessor::expandMacro(const std::string& macro_name, const std::vector<std::string>& args) {
    ...
    // HYGIENE: Generate unique ID for this macro invocation
    // This prevents variable collisions when the same macro is expanded multiple times
    uint64_t unique_id = hygiene_counter.fetch_add(1, std::memory_order_relaxed);
    
    // Apply hygienic renaming to macro body BEFORE parameter substitution
    // This renames all Type:Name declarations to Type:Name_hNNN
    std::string hygienic_body = applyHygienicRenaming(macro.body, unique_id);
...
// Macro Hygiene Implementation (Based on gem3_06.txt research)
// Implements automatic hygienic renaming via gensym pattern from Lisp/Scheme
// Prevents variable collision when macros are invoked multiple times
std::string Preprocessor::applyHygienicRenaming(const std::string& macro_body, uint64_t unique_id) {
    std::string result = macro_body;
    std::string hygiene_suffix = "_h" + std::to_string(unique_id);
    ...
    static const std::set<std::string> reserved_keywords = {
        ...
        "continue", "pick", "int8", "int16", "int32", "int64",
        "uint8", "uint16", "uint32", "uint64", "flt32", "flt64",
        "tbb8", "tbb16", "tbb32", "tbb64", "trit", "tryte", "nit", "nyte",
        "string", "bool", "void", "true", "false"
    };
    
    // Track which variables we need to rename (detect Type:Name declarations)
    std::map<std::string, std::string> rename_map;
    
    // Regex to detect variable declarations: Type:Name pattern
    // Matches things like: int32:temp, %1:val, tbb8:result
    std::regex decl_pattern(R"((\w+):(\w+))");
    
    std::sregex_iterator iter(macro_body.begin(), macro_body.end(), decl_pattern);
    std::sregex_iterator end;
    
    for (; iter != end; ++iter) {
        std::string type_part = (*iter)[1].str();
        std::string var_name = (*iter)[2].str();
        
        // Don't rename if it's a reserved keyword
        if (reserved_keywords.count(var_name) > 0) {
            continue;
        }
        
        // Don't rename macro parameters (%1, %2, etc.)
        // These are caller-controlled and should be left alone
        if (var_name[0] == '%') {
            continue;
        }
        
        // Don't rename if the type itself is a macro parameter
        // (This handles cases like %1:varname where %1 is the type)
        if (type_part[0] == '%') {
            // This is a parameterized type - rename the variable
            rename_map[var_name] = var_name + hygiene_suffix;
        } else {
            // Fixed type - still rename to be safe
            rename_map[var_name] = var_name + hygiene_suffix;
        }
    }
    
    // Apply all renamings
    // We need to be careful to only rename complete identifiers
    for (const auto& [old_name, new_name] : rename_map) {
        // Create regex that matches the identifier as a whole word
        // Positive lookahead/lookbehind to ensure it's not part of another identifier
        std::string pattern = R"(\b)" + old_name + R"(\b)";
        std::regex word_pattern(pattern);
        result = std::regex_replace(result, word_pattern, new_name);
    }
