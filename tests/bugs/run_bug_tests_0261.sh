#!/bin/bash
# Nitpick Bug Regression Tests — v0.26.1
# Runs bug205–bug209 (MEM-005 stack escape detection regression suite).
#   bug205: stack borrow return → ARIA-017 reject
#   bug206: stack-local struct field borrow return → ARIA-017 reject
#   bug207: chained borrow into stack local → ARIA-017 reject
#   bug208: gc-local borrow return → ARIA-017 reject (conservative)
