...
if $ARIA_COMPILER --ast-dump $TEST_DIR/math_utils.npk > /dev/null 2>&1; then
    echo "  ✓ PASS: math_utils.npk parses correctly"
    PASSED=$((PASSED + 1))
else
    echo "  ✗ FAIL: math_utils.npk failed to parse"
    FAILED=$((FAILED + 1))
fi

# Test 2: Minimal use statement parsing
echo "[TEST 2] Use statement parsing..."
if $ARIA_COMPILER --ast-dump $TEST_DIR/test_minimal.npk 2>&1 | grep -q "Use(math_utils)"; then
...
if $ARIA_COMPILER --verbose $TEST_DIR/test_minimal.npk -o /tmp/test_minimal 2>&1 | grep -q "Phase 3: Semantic analysis"; then
...
if $ARIA_COMPILER $TEST_DIR/test_simple_main.npk -o /tmp/test_simple_main > /dev/null 2>&1; then
...
if $ARIA_COMPILER --ast-dump $TEST_DIR/test_namespace_import.npk 2>&1 | grep -q "Use(math_utils)"; then
...
if $ARIA_COMPILER --ast-dump $TEST_DIR/test_selective_import.npk > /dev/null 2>&1; then
...
if $ARIA_COMPILER --ast-dump $TEST_DIR/test_wildcard_import.npk > /dev/null 2>&1; then
...
if $ARIA_COMPILER $TEST_DIR/diamond_test.npk -o /tmp/test_diamond 2>/dev/null && /tmp/test_diamond; then
