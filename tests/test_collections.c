...
void test_array_basic() {
    ...
    AriaResultPtr result = aria_array_new(sizeof(int32_t), 0);
    ...
    // Check initial state
    size_t len = aria_array_length(arr);
    TEST_ASSERT(len == 0, "array_length: new array is empty");
    
    size_t cap = aria_array_capacity(arr);
...
void test_array_push_pop() {
    ...
    // Push elements
    int32_t val1 = 42;
    AriaResultVoid r1 = aria_array_push(arr, &val1);
    TEST_ASSERT(!r1.is_error, "array_push: first element succeeds");
    TEST_ASSERT(aria_array_length(arr) == 1, "array_length: 1 after first push");
    
    int32_t val2 = 99;
    AriaResultVoid r2 = aria_array_push(arr, &val2);
    TEST_ASSERT(!r2.is_error, "array_push: second element succeeds");
    TEST_ASSERT(aria_array_length(arr) == 2, "array_length: 2 after second push");
    
    int32_t val3 = -17;
    aria_array_push(arr, &val3);
    TEST_ASSERT(aria_array_length(arr) == 3, "array_length: 3 after third push");
    
    // Get elements
    AriaResultPtr g1 = aria_array_get(arr, 0);
    TEST_ASSERT(!g1.is_error, "array_get: index 0 succeeds");
    TEST_ASSERT(*(int32_t*)g1.value == 42, "array_get: index 0 has correct value");
    
    AriaResultPtr g2 = aria_array_get(arr, 1);
    TEST_ASSERT(*(int32_t*)g2.value == 99, "array_get: index 1 has correct value");
    
    AriaResultPtr g3 = aria_array_get(arr, 2);
    TEST_ASSERT(*(int32_t*)g3.value == -17, "array_get: index 2 has correct value");
    
    // Pop element
    AriaResultPtr p1 = aria_array_pop(arr);
    TEST_ASSERT(!p1.is_error, "array_pop: succeeds");
    TEST_ASSERT(*(int32_t*)p1.value == -17, "array_pop: returns last value");
    TEST_ASSERT(aria_array_length(arr) == 2, "array_length: 2 after pop");
...
void test_array_bounds() {
    ...
    int32_t val = 42;
    aria_array_push(arr, &val);
    
    // Out of bounds access
    AriaResultPtr get_oob = aria_array_get(arr, 100);
    TEST_ASSERT(get_oob.is_error, "array_get: out of bounds returns error");
    
    // Out of bounds set
    AriaResultVoid set_oob = aria_array_set(arr, 100, &val);
    TEST_ASSERT(set_oob.is_error, "array_set: out of bounds returns error");
    
    // Pop from empty
    aria_array_pop(arr); // Remove the one element
    AriaResultPtr pop_empty = aria_array_pop(arr);
...
void test_map_basic() {
    ...
    AriaResultPtr result = aria_map_new(sizeof(int32_t), sizeof(int32_t), 0);
    ...
    // Check initial state
    size_t len = aria_map_length(map);
...
void test_map_insert_get() {
    ...
    AriaMap* map = aria_map_new_simple(sizeof(int32_t), sizeof(int32_t));
    
    // Insert key-value pairs
    int32_t key1 = 10;
    int32_t val1 = 100;
    aria_map_insert_simple(map, &key1, &val1);
    TEST_ASSERT(aria_map_length(map) == 1, "map_insert: length 1 after first insert");
    
    int32_t key2 = 20;
    int32_t val2 = 200;
    aria_map_insert_simple(map, &key2, &val2);
    TEST_ASSERT(aria_map_length(map) == 2, "map_insert: length 2 after second insert");
    
    // Get values
    int32_t* get1 = (int32_t*)aria_map_get_simple(map, &key1);
    TEST_ASSERT(get1 != NULL, "map_get: key 10 found");
    TEST_ASSERT(*get1 == 100, "map_get: key 10 has value 100");
    
    int32_t* get2 = (int32_t*)aria_map_get_simple(map, &key2);
    TEST_ASSERT(get2 != NULL, "map_get: key 20 found");
    TEST_ASSERT(*get2 == 200, "map_get: key 20 has value 200");
    
    // Get non-existent key
    int32_t key3 = 999;
    int32_t* get3 = (int32_t*)aria_map_get_simple(map, &key3);
...
void test_map_has() {
    ...
    // Check existence
    bool has1 = aria_map_has(map, &key1);
    TEST_ASSERT(has1, "map_has: inserted key exists");
    
    int32_t key2 = 100;
    bool has2 = aria_map_has(map, &key2);
...
void test_map_update() {
    ...
    int32_t key = 5;
    int32_t val1 = 50;
    aria_map_insert_simple(map, &key, &val1);
    
    int32_t* get1 = (int32_t*)aria_map_get_simple(map, &key);
    TEST_ASSERT(*get1 == 50, "map_update: initial value is 50");
    
    // Update with new value
    int32_t val2 = 500;
    aria_map_insert_simple(map, &key, &val2);
    TEST_ASSERT(aria_map_length(map) == 1, "map_update: length still 1 after update");
    
    int32_t* get2 = (int32_t*)aria_map_get_simple(map, &key);
...
void test_map_remove() {
    ...
    int32_t vals[] = {10, 20, 30};
    for (int i = 0; i < 3; i++) {
        aria_map_insert_simple(map, &keys[i], &vals[i]);
    }
    TEST_ASSERT(aria_map_length(map) == 3, "map_remove: 3 pairs inserted");
    
    // Remove middle element
    AriaResultVoid remove_result = aria_map_remove(map, &keys[1]);
    TEST_ASSERT(!remove_result.is_error, "map_remove: removal succeeds");
    TEST_ASSERT(aria_map_length(map) == 2, "map_remove: length 2 after removal");
    
    // Verify removal
    bool has = aria_map_has(map, &keys[1]);
    TEST_ASSERT(!has, "map_remove: removed key doesn't exist");
    
    // Verify others still exist
    bool has0 = aria_map_has(map, &keys[0]);
    TEST_ASSERT(has0, "map_remove: key 1 still exists");
    
    bool has2 = aria_map_has(map, &keys[2]);
...
void test_map_clear() {
    ...
    TEST_ASSERT(aria_map_length(map) == 10, "map_clear: 10 pairs inserted");
    
    // Clear map
    aria_map_clear(map);
    TEST_ASSERT(aria_map_length(map) == 0, "map_clear: length 0 after clear");
    
    // Verify empty
    int32_t key = 5;
    bool has = aria_map_has(map, &key);
...
void test_map_string_keys() {
    ...
    AriaMap* map = aria_map_new_simple(sizeof(char*), sizeof(int32_t));
    ...
    TEST_ASSERT(aria_map_length(map) == 2, "map_string: 2 string keys inserted");
