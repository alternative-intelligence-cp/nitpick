...
void test_arena_creation() {
    ...
    AriaGenArenaResult res = npk_gen_arena_new(sizeof(Neuron), 4);
    assert(res.error_code == ARIA_GEN_ARENA_OK);
    assert(res.arena != NULL);
    assert(npk_gen_arena_capacity(res.arena) == 4);
    assert(npk_gen_arena_count(res.arena) == 0);
    
    npk_gen_arena_destroy(res.arena);
...
void test_alloc_and_get() {
    ...
    AriaGenArenaResult arena_res = npk_gen_arena_new(sizeof(Neuron), 4);
    npk_gen_arena* arena = arena_res.arena;
    
    // Allocate neuron
    Neuron n1 = {.id = 42, .value = 3.14};
    AriaHandleAllocResult alloc_res = npk_gen_arena_alloc(arena, &n1);
    assert(alloc_res.error_code == ARIA_GEN_ARENA_OK);
    assert(!npk_handle_is_invalid(alloc_res.handle));
    assert(alloc_res.handle.index == 0);
    assert(alloc_res.handle.generation == 1);
    
    // Get neuron
    AriaElemGetResult get_res = npk_gen_arena_get(arena, alloc_res.handle);
...
void test_stale_handle_detection() {
    ...
    AriaHandleAllocResult alloc_res = npk_gen_arena_alloc(arena, &n1);
    npk_handle h = alloc_res.handle;
    
    // Free neuron
    AriaGenArenaError free_err = npk_gen_arena_free(arena, h);
    assert(free_err == ARIA_GEN_ARENA_OK);
    
    // Try to get freed neuron (should fail with stale handle)
    AriaElemGetResult get_res = npk_gen_arena_get(arena, h);
    assert(get_res.error_code == ARIA_GEN_ARENA_ERR_STALE_HANDLE);
    assert(get_res.elem == NULL);
    
    // Verify stale_gets counter incremented
    AriaGenArenaStats stats = npk_gen_arena_stats(arena);
...
void test_free_list_recycling() {
    ...
    AriaHandleAllocResult h1_res = npk_gen_arena_alloc(arena, &n1);
    AriaHandleAllocResult h2_res = npk_gen_arena_alloc(arena, &n2);
    
    npk_handle h1 = h1_res.handle;
    npk_handle h2 = h2_res.handle;
    
    assert(h1.index == 0);
    assert(h2.index == 1);
    
    // Free first neuron
    npk_gen_arena_free(arena, h1);
    assert(npk_gen_arena_count(arena) == 1);
    assert(npk_gen_arena_free_count(arena) == 1);
    
    // Allocate new neuron (should reuse slot 0)
    Neuron n3 = {.id = 3, .value = 3.0};
    AriaHandleAllocResult h3_res = npk_gen_arena_alloc(arena, &n3);
    npk_handle h3 = h3_res.handle;
    
    assert(h3.index == 0);  // Reused slot!
    assert(h3.generation == 2);  // But generation incremented!
    assert(npk_gen_arena_free_count(arena) == 0);
    
    // Old handle h1 is now stale
    AriaElemGetResult stale_res = npk_gen_arena_get(arena, h1);
    assert(stale_res.error_code == ARIA_GEN_ARENA_ERR_STALE_HANDLE);
    
    // New handle h3 works
    AriaElemGetResult valid_res = npk_gen_arena_get(arena, h3);
...
void test_arena_growth() {
    ...
    AriaGenArenaResult arena_res = npk_gen_arena_new(sizeof(Neuron), 2);
    npk_gen_arena* arena = arena_res.arena;
    
    assert(npk_gen_arena_capacity(arena) == 2);
    ...
    npk_gen_arena_alloc(arena, &n1);
    npk_gen_arena_alloc(arena, &n2);
    npk_gen_arena_alloc(arena, &n3);  // Should trigger growth
    
    assert(npk_gen_arena_capacity(arena) == 4);  // 2x growth
    assert(npk_gen_arena_count(arena) == 3);
...
void test_statistics() {
    ...
    // Alloc
    AriaHandleAllocResult h_res = npk_gen_arena_alloc(arena, &n);
    npk_handle h = h_res.handle;
    
    // Get
    npk_gen_arena_get(arena, h);
    
    // Free
    npk_gen_arena_free(arena, h);
