...
detect_environment() {
    ...
    # Check npkc compiler
    if [ ! -x "$NPKC" ]; then
        echo -e "${RED}Error: npkc compiler not found at $NPKC${NC}"
        echo -e "${YELLOW}Build with: cd build && cmake .. && make -j\$(nproc)${NC}"
        exit 1
    fi
    echo -e "  Compiler: ${GREEN}$NPKC${NC}"
...
test_emit_ptx() {
    ...
    # Find compilable .npk files in gpu test directory
    local aria_files=("$SCRIPT_DIR"/*.npk)
    if [ ${#aria_files[@]} -eq 0 ] || [ ! -f "${aria_files[0]}" ]; then
        skip_test "--emit-ptx codegen" "no .npk files found"
        return
    fi

    for aria_file in "${aria_files[@]}"; do
        local name
        name=$(basename "$aria_file" .npk)
        local ptx_out="$TMP_DIR/${name}.ptx"

        log "Compiling: $aria_file → PTX"

        # Compile to PTX (suppress debug output, capture errors)
        if "$NPKC" "$aria_file" --emit-ptx --gpu-arch="$GPU_ARCH" -o "$ptx_out" 2>"$TMP_DIR/ariac_err.log"; then
