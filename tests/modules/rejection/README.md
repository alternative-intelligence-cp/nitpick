...
| `mod_private_use.npk` | `NITPICK-RESOLVE-003` — `use nested.internal;` over an inline module: a private member is private from either spelling (D-273 §3, 1.5.4c; the qualified call's refusal is the checker's, in `tests/types/rejection/mod_private_qualified.npk`) |
