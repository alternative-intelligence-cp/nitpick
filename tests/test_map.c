...
int main() {
    ...
    AriaMap* map = npk_map_new_simple(sizeof(int64_t), sizeof(char*));
    printf("✓ Created map\n");
    ...
    npk_map_insert_simple(map, &key3, &val3);
    ...
    // Test has
    printf("\nTesting 'has':\n");
    printf("  has(42): %s\n", npk_map_has(map, &key1) ? "true" : "false");
    printf("  has(100): %s\n", npk_map_has(map, &key2) ? "true" : "false");
    int64_t missing = 999;
    printf("  has(999): %s\n", npk_map_has(map, &missing) ? "true" : "false");
    
    // Test get
    printf("\nTesting 'get':\n");
    const char** result1 = (const char**)npk_map_get_simple(map, &key1);
    printf("  get(42): \"%s\"\n", *result1);
    
    const char** result2 = (const char**)npk_map_get_simple(map, &key2);
    printf("  get(100): \"%s\"\n", *result2);
    
    const char** result3 = (const char**)npk_map_get_simple(map, &key3);
    printf("  get(7): \"%s\"\n", *result3);
    
    // Test update (insert existing key)
    printf("\nUpdating existing key:\n");
    const char* new_val1 = "Updated!";
    npk_map_insert_simple(map, &key1, &new_val1);
    const char** updated = (const char**)npk_map_get_simple(map, &key1);
    printf("  get(42) after update: \"%s\"\n", *updated);
    printf("  length after update: %zu (should still be 3)\n", npk_map_length(map));
    
    // Test remove
    printf("\nRemoving key 100:\n");
    npk_map_remove(map, &key2);
