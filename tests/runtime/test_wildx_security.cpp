...
static void test_basic_cycle() {
    ...
    WildXGuard guard = aria_asm_finalize(a);
...
static void test_code_hash() {
    ...
    uint64_t computed = aria_wildx_verify_hash(&guard);
...
static void test_hash_uniqueness() {
    Assembler* a1 = aria_asm_create();
    aria_asm_mov_r64_imm64(a1, REG_RAX, 1);
    aria_asm_ret(a1);
    WildXGuard g1 = aria_asm_finalize(a1);
    
    Assembler* a2 = aria_asm_create();
    aria_asm_mov_r64_imm64(a2, REG_RAX, 2);
    aria_asm_ret(a2);
    WildXGuard g2 = aria_asm_finalize(a2);
...
static void test_guard_pages_normal() {
    aria_wildx_enable_guard_pages(true);
    
    Assembler* a = aria_asm_create();
    aria_asm_mov_r64_imm64(a, REG_RAX, 77);
    aria_asm_ret(a);
    ...
    aria_wildx_enable_guard_pages(false);
...
static void test_quota_enforcement() {
    ...
    WildXGuard g4 = aria_alloc_exec(100);
    TEST_ASSERT(g4.ptr != nullptr, "quota - post-free alloc");
    aria_free_exec(&g4);
    
    // Restore default quota
    aria_wildx_set_quota(64ULL * 1024 * 1024);
...
static void test_execute_with_arg() {
    ...
    aria_asm_add_r64_r64(a, REG_RAX, REG_RAX);
    aria_asm_ret(a);
    
    WildXGuard guard = aria_asm_finalize(a);
    TEST_ASSERT(guard.ptr != nullptr, "execute with arg");
    
    int64_t result = aria_asm_execute_i64(&guard, 21);
