...
TEST_CASE(asm_buffer_create_destroy) {
    CodeBuffer* buf = npk_asm_buffer_create(1024);
    ...
    npk_asm_buffer_destroy(buf);
}

TEST_CASE(asm_buffer_emit_byte) {
    CodeBuffer* buf = npk_asm_buffer_create(16);
    ASSERT(buf != nullptr, "Buffer creation should succeed");
    
    npk_asm_emit_byte(buf, 0x90);  // NOP
...
TEST_CASE(asm_buffer_emit_i32) {
    ...
    npk_asm_emit_i32(buf, 0x12345678);
...
TEST_CASE(asm_buffer_growth) {
    CodeBuffer* buf = npk_asm_buffer_create(4);  // Small initial capacity
    ASSERT(buf != nullptr, "Buffer creation should succeed");
    
    // Fill beyond initial capacity
    for (int i = 0; i < 100; ++i) {
        npk_asm_emit_byte(buf, (uint8_t)i);
    }
...
TEST_CASE(asm_label_create_unbound) {
    AsmLabel label = npk_asm_label_create();
    ASSERT(!npk_asm_label_is_bound(&label), "New label should be unbound");
    ASSERT(label.num_patches == 0, "No patches initially");
}

TEST_CASE(asm_new_label) {
    Assembler* asm_ctx = npk_asm_create();
    ASSERT(asm_ctx != nullptr, "Assembler creation should succeed");
    
    int label1 = npk_asm_new_label(asm_ctx);
    int label2 = npk_asm_new_label(asm_ctx);
...
TEST_CASE(asm_bind_label) {
    ...
    int label = npk_asm_new_label(asm_ctx);
    
    // Emit some bytes
    npk_asm_emit_byte(asm_ctx->buffer, 0x90);
    npk_asm_emit_byte(asm_ctx->buffer, 0x90);
    
    npk_asm_bind_label(asm_ctx, label);
    
    ASSERT(npk_asm_label_is_bound(&asm_ctx->labels[label]), "Label should be bound");
...
TEST_CASE(asm_mov_r64_imm64) {
    ...
    // MOV RAX, 42
    npk_asm_mov_r64_imm64(asm_ctx, REG_RAX, 42);
...
TEST_CASE(asm_mov_r64_r64) {
    ...
    // MOV RAX, RBX
    npk_asm_mov_r64_r64(asm_ctx, REG_RAX, REG_RBX);
...
TEST_CASE(asm_add_r64_r64) {
    ...
    // ADD RAX, RBX
    npk_asm_add_r64_r64(asm_ctx, REG_RAX, REG_RBX);
...
TEST_CASE(asm_sub_r64_r64) {
    ...
    // SUB RAX, RBX
    npk_asm_sub_r64_r64(asm_ctx, REG_RAX, REG_RBX);
...
TEST_CASE(asm_jmp_backward) {
    ...
    int label = npk_asm_new_label(asm_ctx);
    npk_asm_bind_label(asm_ctx, label);  // Bind at offset 0
    
    npk_asm_emit_byte(asm_ctx->buffer, 0x90);  // NOP (offset 0)
    npk_asm_jmp(asm_ctx, label);  // Jump back to offset 0
...
TEST_CASE(asm_jmp_forward_patching) {
    ...
    npk_asm_jmp(asm_ctx, label);  // Forward jump (unresolved)
    ...
    npk_asm_emit_byte(asm_ctx->buffer, 0x90);  // NOP
    npk_asm_bind_label(asm_ctx, label);  // Bind at offset 6
...
TEST_CASE(asm_conditional_jumps) {
    ...
    npk_asm_je(asm_ctx, label1);
    npk_asm_jne(asm_ctx, label2);
...
TEST_CASE(jit_return_constant) {
    ...
    WildXGuard guard = npk_asm_finalize(asm_ctx);
...
TEST_CASE(jit_identity_function) {
    ...
    npk_asm_mov_r64_r64(asm_ctx, REG_RAX, REG_RDI);
    ...
    int64_t result = npk_asm_execute_i64(&guard, 100);
...
TEST_CASE(jit_add_two_numbers) {
    ...
    // ADD RAX, RSI (add b)
    npk_asm_add_r64_r64(asm_ctx, REG_RAX, REG_RSI);
    ...
    int64_t result = npk_asm_execute_i64_i64(&guard, 10, 32);
...
TEST_CASE(jit_subtract_numbers) {
    ...
    npk_asm_mov_r64_r64(asm_ctx, REG_RAX, REG_RDI);
    npk_asm_sub_r64_r64(asm_ctx, REG_RAX, REG_RSI);
    npk_asm_ret(asm_ctx);
    
    WildXGuard guard = npk_asm_finalize(asm_ctx);
    ASSERT(guard.ptr != nullptr, "Finalization should succeed");
    
    int64_t result = npk_asm_execute_i64_i64(&guard, 100, 58);
...
TEST_CASE(jit_multiply_numbers) {
    ...
    npk_asm_mov_r64_r64(asm_ctx, REG_RAX, REG_RDI);
    npk_asm_imul_r64_r64(asm_ctx, REG_RAX, REG_RSI);
    npk_asm_ret(asm_ctx);
    
    WildXGuard guard = npk_asm_finalize(asm_ctx);
    ASSERT(guard.ptr != nullptr, "Finalization should succeed");
    
    int64_t result = npk_asm_execute_i64_i64(&guard, 6, 7);
...
TEST_CASE(jit_conditional_branch) {
    ...
    int label_return_a = npk_asm_new_label(asm_ctx);
    
    // CMP RDI, RSI (compare a and b)
    npk_asm_cmp_r64_r64(asm_ctx, REG_RDI, REG_RSI);
    
    // JE label_return_a (if a == b, return a)
    npk_asm_je(asm_ctx, label_return_a);
    
    // Return b (RSI) - not equal case
    npk_asm_mov_r64_r64(asm_ctx, REG_RAX, REG_RSI);
    npk_asm_ret(asm_ctx);
    
    // Return a (RDI) - equal case
    npk_asm_bind_label(asm_ctx, label_return_a);
    npk_asm_mov_r64_r64(asm_ctx, REG_RAX, REG_RDI);
    npk_asm_ret(asm_ctx);
    ...
    int64_t result1 = npk_asm_execute_i64_i64(&guard, 42, 42);
    ASSERT(result1 == 42, "select(42, 42) should return 42");
    
    // Test not equal case: should return second argument
    int64_t result2 = npk_asm_execute_i64_i64(&guard, 30, 70);
...
TEST_CASE(jit_prologue_epilogue) {
    ...
    npk_asm_prologue(asm_ctx, 0);  // No stack variables
    npk_asm_mov_r64_imm64(asm_ctx, REG_RAX, 42);
    npk_asm_epilogue(asm_ctx);
...
TEST_CASE(jit_error_unbound_label) {
    ...
    int label = npk_asm_new_label(asm_ctx);
    npk_asm_jmp(asm_ctx, label);  // Forward jump, never bound
    npk_asm_ret(asm_ctx);
    
    WildXGuard guard = npk_asm_finalize(asm_ctx);
    ASSERT(guard.ptr == nullptr, "Finalization should fail");
    ASSERT(npk_asm_has_error(asm_ctx), "Should have error");
