...
int main() {
    ...
    AriaExecutorHandle exec = npk_executor_create();
    if (exec) {
        std::cout << "✓ PASS\n";
        tests_passed++;
    } else {
    ...
    AriaExecutorHandle global1 = npk_get_global_executor();
    AriaExecutorHandle global2 = npk_get_global_executor();
    ...
    AriaFutureHandle fut = npk_future_create(sizeof(int64_t));
    int64_t value = 99;
    npk_future_set_value(fut, &value, sizeof(value));
    void* result = npk_future_get_value(fut);
    ...
    AriaTaskId taskId = npk_executor_spawn(exec, coro1);
    if (taskId > 0) {
        std::cout << "✓ PASS\n";
        tests_passed++;
    } else {
    ...
    npk_executor_run(exec);  // This will run the task we just spawned
    ...
    bool initially_not_done = !npk_coro_done(test_coro);  // Should not be done initially
    npk_coro_resume(test_coro);
    bool after_resume_done = npk_coro_done(test_coro);     // Should be done after resume
