...
private:
    ...
    // Macro Hygiene: Atomic counter for gensym (thread-safe unique IDs)
    // Implements automatic hygienic renaming to prevent variable collision
    // (Research: gem3_06.txt - Gemini's macro hygiene solution)
    static std::atomic<uint64_t> hygiene_counter;
    ...
    // Macro Hygiene: Apply automatic variable renaming to prevent collisions
    // Detects Type:Name patterns and appends _hNNN suffix for uniqueness
    std::string applyHygienicRenaming(const std::string& macro_body, uint64_t unique_id);
