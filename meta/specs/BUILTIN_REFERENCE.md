...
| `atomic_from_ptr::<T>` | `wild T-> → atomic<T>` | Aliases existing memory AS an atomic (D-033), rather than allocating storage (which `atomic<T>:x;` does) — the element type comes from the **turbofish** (D-064's expression-position type-argument syntax; D-187). **`wild`-context only** (D-187): it fabricates an atomic view over an address the type system did not allocate, exactly as privileged as the `#wild_ptr` that produced the address. **Fused-only**: legal solely as an atomic method's receiver — `atomic_from_ptr::<int64>(p).load()` — where the emitter dispatches through the pointer itself; a declaration or assignment storing the result is refused (TYPE-007), which makes "aliased atomics are never stored" structural rather than a discipline. | **never fails** (traps on misuse) |
