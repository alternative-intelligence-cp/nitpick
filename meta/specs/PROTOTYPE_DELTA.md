...
| §14.2.1, plus `function_specs.txt` and `mod_system_specs.txt` | "functions declared inside `extern` blocks DO NOT return `Result<T>`… you do not use `raw`" | **Reversed.** An `extern` function's type is `Result<T>` like every other function's, and the `fails on` / `never fails` contract decides what goes in the error field — D-002, implemented as D-114. A caller handles a foreign call exactly as a domestic one, which is the point: never having to ask whether *this* function needs error handling. (D-149 then moved `extern` out of process entirely, and under D-163 `raw` is licensed only by `never fails` — which a driver call never carries — so the uniform spellings there are `relay`/`?|`/`?!`) |
