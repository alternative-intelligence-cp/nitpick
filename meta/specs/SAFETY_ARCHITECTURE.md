...
*   **K-Semantics on `exit`**: The failsafe and `main` are the only places allowed to call `exit`. Nitpick enforces that no unchecked manual memory (`wild`/`wildx`) is active upon `exit`, and — per D-062 — that no task frames are live either. If a leak or a live task exists during an exit attempt, it traps back into the failsafe to enforce cleanup.
