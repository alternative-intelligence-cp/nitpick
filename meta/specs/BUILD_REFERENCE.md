...
| `verify` | the compiler under test, z3 | (1.5.0, D-218; 1.5.4: a test carrying `expect-error:` lines says the VERIFIED build refuses with exactly that set — an undischarged `prove` — and ends there) compiled with `--obligations`, its rows decided by the pinned z3 under the pinned profile, the (kind, verdict) counts of the test's OWN module equal to its `expect-obligation:` lines exactly, then the VERIFIED build emitted with that run's manifest, cross-checked, linked and run at -O0 and under opt -O2 with `expect-exit:` met both times; since 1.5.6 the stage ends with the FLOOR'S rows: `tools/floorspec.npk` (built with the snapshot) writes `verify/floor/` from `runtime/npkrt.ll` and `runtime/npkrt.spec`, z3 decides it, an `open` row fails by name, and the rows are held to the committed `runtime/npkrt.obligations` |
...
| `explore` | the compiler under test, the explored floor, the shim | (1.5.7 step 1; D-212, D-298…D-301) every `// stress:` program under the entry's paths says whether it is explored — `// explore: N [d=D]` or `// explore: no <reason>` (D-299; neither is `explore-unmarked`, a red run by name) — and a marked one is emitted, scanned, assembled, linked against the EXPLORED floor (`runtime/npkrt.ll` through the one transformer `npkg/explore.npk`: a scheduling point before every atomic step, every syscall routed) and the hand-written IR shim (`runtime/explore/npkx.ll`, itself under the floor's belts), and run under N seeds of the shim's PCT scheduler: a measuring run first (seed 0, no change points) gives k, every seeded run is held to the program's `expect-exit:` and to no verdict of the shim (`DEADLOCK`, `STEP BUDGET`, `MMAP`; the quiescence oracles from step 3; `SHIM FAULT`, a fault inside the shim's own code, from 1.5.8 step 3, K-13), the first seed runs twice and its two schedule hashes must agree (`explore-replay-differs`), a `// explore-seed: S` runs first forever, and the stage prints PCT's per-run bound `1/(n·k^(d−1))` per unit; a failure carries its replay line (`NPKX_SEED=… NPKX_K=… NPKX_D=… NPKX_TRACE=1 <binary> < /dev/null`). Then THE CONTROLS (steps 3–4; X-10, X-15): every `runtime/explore/controls/*.ctl` plants a defect in the floor by exact-line substitution (one or more `old:`/`new:` pairs), names a program and the verdict the explorer must reach within N seeds — a word of the shim, `wrong-exit`, `exit N` or `late N` (the virtual run time, `vrun=`) — optionally DIRECTED at up to four atomic sites (`preempt-at:`, resolved against the patched floor's `sites.txt`, `NPKX_PREEMPT1..4` to the shim) and HELD at up to four more (`hold-at:`, 1.5.8 step 2c, resolved the same way, `NPKX_HOLD1..4`: the first arrival waits until another thread passes the same site); the patched floor is transformed, counted, assembled and linked in the run's build directory, a measuring run gives k, and a control the explorer never reaches the verdict of is `explore-control-blind`; a file the grammar cannot read `-malformed`, `old` lines that do not occur exactly once `-unmatched`, a site that names no single atomic row `-site-unmatched`/`-site-kind`; a SPEC control (`spec-old:`/`spec-new:`, step 5) plants a false caller hypothesis in `runtime/npkrt.spec` and expects `ASSUMPTION`; a PROGRAM control (`program-old:`/`program-new:`, step 6, X-21) plants a defect in the SOURCE of the program it names, compiled from the run's build directory. THE PROGRAM'S OWN STEPS (step 6, X-20): every unit's emitted IR, a control's program included, goes through the same transformer's program mode before `llc` (`explored --program IN OUT SITES` in the harness, in-process in npkg) -- a point before every atomic step of its defines (`atomic<T>`, `atomic_from_ptr`), every `sys` call's `@npk_sys6(` routed, its sites numbered from 1,000,000 -- and is held to the counting belt (`explore-step-escapes`, "the program holds N step line(s) …") and, in the harness, to its site map. THE SPEC'S CALLER HYPOTHESES (step 5, D-302): the transformer writes one ENTRY CHECKER per section of the spec that has rows and a `requires`/`objects`/`views` clause (`@"npkx.req.<symbol>"`, called as the symbol's first instruction, the clauses evaluated over the entry state in i128 — the unsigned reading, a load below the null page reading 0) and `assumptions.txt` beside `sites.txt` (one row per clause, `checked` or `listed` with the reason); a false clause at any call of any explored schedule is the shim's verdict `ASSUMPTION <symbol>: <clause>`; both runners hold the rows to a second reader (`explore-assumptions`: the harness classifies every clause again by the generator's rules, npkg counts the facts per section) and to the explored text (every checked symbol's checker defined and called first), print the counts and every listed clause by name, and TCB.md §4d's generated table carries the same in its last column. WHAT NO POINT CAN PRECEDE (1.5.8 step 2b; DEF-64): every syscall the floor's `module asm` census reads (the clone trampoline's `clone` and `exit`, the stop handler's restorer `rt_sigreturn`) is stated with its reason in `runtime/explore/unrouted.txt`, held to the census exactly in both runners (`explore-asm-unlisted`, `explore-asm-stale`, `explore-asm-malformed`) and printed on every run |
