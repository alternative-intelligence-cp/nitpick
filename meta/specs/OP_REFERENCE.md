...
| `_?` | Drop | Desugars to `drop expr` — the "void call" of a `never fails`, `NIL`-returning function (**D-163, settled; the licence flips at 1.1.2**; no longer discards an error). | `_? my_func();` |
| `_!` | Raw | Desugars to `raw expr` — unwrap the value of a `never fails` call (**D-163, settled; the licence flips at 1.1.2**: a checked, zero-cost unwrap, not a bypass). | `val = _! my_func();` |
