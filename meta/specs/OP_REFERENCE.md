...
| `_?` | Drop | Desugars to `drop expr` — the "void call" of a `never fails`, `NIL`-returning function (**D-163**; the `void` call of a `never fails`, `NIL` callee — refused otherwise, `TYPE-042`). | `_? my_func();` |
