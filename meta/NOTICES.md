...
| 51 | 2026-09-25 | | 1.6.0 step 0 (the bring-up gate PLANNED execution-grade, measured first; D-319 NIKOS the IKOS candidate, D-320 the gate's decision rule, D-321 Alive2's budget and solver pin; the gate's tools README, census script and probe log) | no language change; every ladder row unchanged from `c3bdae2`; both manifests unchanged; sent to `nitpick-libs_s6` (the seat since the 1.5 close); next: 1.6.0 step 1 (the pinned builds, outside the tree), no floor move planned |
