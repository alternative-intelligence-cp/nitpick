...
|---|---|---|
| **A** | Whole-program runtime-error absence — OOB, div-by-zero, null/uninitialized reads — by sound abstract interpretation over OUR emitted IR | **NIKOS** (D-322, 1.6.0's gate, 2026-09-25: seven of eight planted defects found; Clam/Crab decided out — its memory analyses abort on our by-value aggregate arguments, soundly, and it has no `div-zero` check) |
...
| # | Topic | Gated on |
|---|---|---|
| 1.6.0 ✅ | **The bring-up gate — DONE 2026-09-25 (D-322: NIKOS)** — both leg-A candidates built at pinned commits and run against the same three emissions: `dyn_slots.npk` (small; aggregates, `dyn`), `extern_c_driver.npk` (the richest idiom mix — bridge, coroutines, deadlines), and `src/npkc.npk`'s own emission (scale). Measured per engine: (1) does it INGEST our LLVM-20 opaque-pointer IR at all, and if not, what is the port distance — IKOS sits at LLVM 14 today, across the opaque-pointer break, which is the NIKOS-shaped port; Clam targets 15 with support to 18, and our conservative instruction vocabulary may cross 18→20 textually for free; (2) alarm count and quality on those three programs; (3) wall-clock and memory; (4) determinism controllability — fixed seeds/options, verdicts stable across runs and cwds (the D-204 discipline applied to a new tool). **One engine wins by these measurements; the other is decided out.** Alive2 is built at the 20.1.2-matching commit in the same subcycle and smoke-run on one program's pre/post-opt pair. If the winning engine needs a port, the port is THIS cycle's work and starts here — scoped by the gate's measurements, not assumed. | D-233 *[2026-09-25, the user: after cycle 1.6 comes a testing-and-benchmarking phase, and a finding read as benign at the gate — the Alive2 inter-procedural class, E-7's emission facts — is recorded and re-examined there rather than stopping the line.]* |
| 1.6.1 | **Leg A as a standing stage — NIKOS** (`1.6.1.md`, PLANNED execution-grade 2026-09-25 at 1.6.0 step 5): the emission states its own data layout first (E-8, D-322 (5)); then TRANSFORM-FREE ingestion (the importer learns `select`, `switch`, the atomics and constant expressions, the driver reads the emission's own bitcode, a belt holds the analyzer's input to the emission — D-319, measured at the gate as one and eight sites of difference); then the port item by item in the user's NIKOS tree, each a pin move measured by the gate's runner (the three defects found by reading; the overflow-intrinsic model with its range split; `llvm.assume` with the second decider of each z3 discharge; the signedness from the operation; the entry seeding; the allocator-entry model and the kernel-effect table as the floor's model — TCB.md's enumeration, a new consumer; the executor wall); then the stage `analyze` in both runners over every real-backend program's plain emission and the compiler's, `nitpick.toml`'s `[verify.nikos]` grown into the pinned-tool table (commit, three digests, the option list every invocation is built from), and the **alarm ledger** `nitpick.alarms` — one row per site with a class and a disposition (true-defect / imprecision-accepted / fact-missing / model-pending), a NEW site on an unchanged tree failing the stage, a vanished one re-recorded in the commit that removed it, the planted controls the stage's standing self-check. The whole-program forms stay the gate runner's measurement of the wall; the compiler's whole form and its cadence are 1.6.3's. | 1.6.0 |
