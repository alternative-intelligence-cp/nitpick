...
| **0.6** | **Macros and comptime** — bounded expansion (D-057), `comptime` evaluation, derive. **Planned in detail** (`0.6/`), seven subcycles. **The last cycle of Phase A**, and the first to add a pass that REWRITES the AST every other pass reads. |
