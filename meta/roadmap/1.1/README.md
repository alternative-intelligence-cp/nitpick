...
| **1.1.12** | **The reactor** — **a: DONE (D-184, B-3a closed)**: epoll WITHOUT timerfd — `epoll_pwait` as the armed executor's idle wait, carrying the sleeper deadline; eventfd as the cross-thread wake channel; EPOLLONESHOT with the task frame as payload; `suspend_io` builtin + prelude `io_ready` + deferred `io_unwatch` (registration lives exactly as long as the wait); ctl-failure = due-now, the caller's retried syscall reports; **the task-identity rule** — every waiter registration (channels and locks included) resolves `cur_task`, closing a latent nested-wait lost-wakeup (`nested_wait.npk`); `sys` takes pointers (`ptrtoint` at the trampoline). io_uring refused before Astrée by decision. **b: DONE (D-185)**: `OwnedFd` (drop = close; move-only; channel-refused, spawn-move legal), `Path`/`path_parse`, `Reader`/`Writer` + `ByteReader`/`ByteWriter` retry loops over `io_ready`, `Whence` seek, TYPE-048 async-conformance, the pointer-receiver await fix, and the S-1 string-view hazard (OPEN_DECISIONS, pre-Astrée). **c: DONE**: TextWriter/TextReader/LineBufWriter as generic wrappers (buffering is a TYPE — no mode fields), LineEnding, split-CR translation, `string_bytes`, std ctors over owned dups (blocking-std: the one stated D-071 exception) — plus the raw-template-field-resolve class fixed (3 sites), the struct-literal lookahead, and substitution-over-recorded-symbol for awaited bound calls. **d: DONE — 1.1.12 COMPLETE**: async behind `dyn` via resume slots + the frame-size tail (`npk.fsz` globals); object-safety rule 4 narrowed (async + `Self->` is safe); rule 2 admits `Self->` receivers; the sync thunk's by-value-receiver latent fixed; `dyn_stream.npk` proves one erased call, two writers, two byte patterns | B-3a |
