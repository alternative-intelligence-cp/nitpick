...
**DEF-49 — FIXED at 1.5.6 step 4 (2026-09-11) (found by that step, on the
b4 worktree, reading `npk_read_file`'s growth to specify it) — `read_file`
...
**DEF-47 — OPEN (found by 1.5.6's planning; the design is S-69, landing at
1.5.6 step 1) — two threads trapping at once run two `failsafe`s, and a
trap during another thread's `failsafe` exits the process mid-safing.**
