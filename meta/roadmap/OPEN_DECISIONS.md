...
**DEF-26 (found by 1.5.4's PLANNING on `b2f7d94`, 2026-09-06, by a probe;
latent since 1.5.0) — a guard site inside a `pick` EXPRESSION arm has no
obligation row.** `expr_children`'s `ExprPickExpr` arm lists only the
...
**DEF-27 (the 1.5.3 handoff's recorded weakness; confirmed at 1.5.4's
planning) — a callee's parameter name that collides with an ADDRESS-TAKEN
caller local binds unnamed in `enc_under`**, because `bind_define` consults
...
**DEF-28 (found by 1.5.4's PLANNING on `b2f7d94`, 2026-09-06, by a probe) — a
non-positive LITERAL step of a counted loop compiles and traps at run time.**
