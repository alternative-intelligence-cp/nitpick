...
| 1.0.9a | **Allocas are hoisted to the entry block** — a latent `alloca`-in-a-loop stack overflow that part one's added code tripped: the seed-built `npkc` segfaulted compiling `src/` in `expand_audit` (~100k expressions × a per-iteration alloca over 8 MB). Every alloca goes to the entry block, once per call, in both codegens by the same transform so the fixpoint holds | 1.0.9 |
| 1.0.9 | **The 1.0 tail, part one: the decisions, the instrument, the conversions** — D-165–D-172 settled (the user took every recommendation: globals as constants, `for` over an `Iterator` with `Self->` receivers, `?|` struck as a second `?`, `&{ }` via `ToString`, non-scalar `==` refused, `(`Type`)` grouping, every impl names its target, `Trait.method(recv, …)`), three amendments (D-008, D-019, D-095); `check_rung_names_open_cycle`; the CONVERT rows and the checker holes the audit found | 1.0.7, 1.0.8 |
| 1.0.9b | **The tail, part two: the trait and grammar decisions land** — parenthesised types (D-170), every impl names its target (D-171), `Trait.method(recv, …)` (D-172), the `dyn` cast and a `dyn` over a family instance lowered | 1.0.9 |
| 1.0.9c | **The tail, part three: the value and pattern lowerings** — multi-payload variants, struct destructuring, a call through a function-valued member, `comptime` at run time, the pipe, non-scalar casts and the remaining builtins enumerated, `==` on a non-scalar refused at the checker (D-169), `#unreachable()` | 1.0.9b |
| 1.0.9d | **The tail, part four: the library-facing decisions, and the cycle closes** — module-level bindings (D-165), `&{ }` through `ToString` (D-168), `for` over an `Iterator` (D-166), the `Hash` and `Ord`-on-enum generators; `rg '"1\.0"' src/backend` returns nothing and **cycle 1.0 closes** | 1.0.9c |
