...
| 1.0.5b | **`dyn` lowering** — the backend half of D-158/D-159: per-(impl,trait) vtables in trait-declaration order, per-impl adapter thunks, construction, call-through-slot, and widening as a value rebuild; retires the `TY_DYN` rung. Also closes D-159's tie-break hole by keying the canonical bound order on a MODULE-QUALIFIED name (two same-named traits currently tie and fall back to declaration index, which shifts under an unrelated edit) | 1.0.5 |
