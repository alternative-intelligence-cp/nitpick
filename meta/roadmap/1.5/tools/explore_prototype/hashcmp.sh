...
for f in $(grep -l '^// stress:' tests/backend/programs/*.npk | sort); do
  ...
  for s in $(seq 1 "$SEEDS"); do
    ...
    vc=$(grep -oE 'npkx: (DEADLOCK|STEP BUDGET|MMAP|LOST[A-Z -]*|ASSUMPTION|SHIM FAULT)' "$OUT/$name.tc" | head -1 | tr -cd 'A-Z'); vi=$(grep -oE 'npkx: (DEADLOCK|STEP BUDGET|MMAP|LOST[A-Z -]*|ASSUMPTION|SHIM FAULT)' "$OUT/$name.ti" | head -1 | tr -cd 'A-Z')
