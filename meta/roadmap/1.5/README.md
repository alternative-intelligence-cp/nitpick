...
| 1.5.4d | **The file-module import's scope and the inline module's error identities (D-274, D-275; S-49/S-50 ratified 2026-09-10) — PLANNED 2026-09-10 (`1.5.4d.md`, execution-grade; proceeds under R-3, a PAIR check of a qualified arm against the constants the program declares, and R-4, every arm over an `Error` selector one of `(*)`, `(Name)` bound to a constant or `(file.Name)`; S-51 — imports inside an inline module, found by the planning as a silent no-op — gates step 0b alone), ahead of 1.5.4b as 1.5.4c was**: a member-less `mod:name;` import carries the loaded file's scope so `name.f()` resolves as `use "./name.npk" as name;` does (one `inner` field, D-273 §4); an error constant declared inside an inline module stays the FILE's, and a `failsafe` arm's qualifier naming no module the program knows is RESOLVE-002 at the arm rather than a silent non-match. Small: the loader's `graph_load_file_module` and the arm checks; both measured at 1.5.4c as having no user in the tree | 1.5.4 |
