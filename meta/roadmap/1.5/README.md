...
| 1.5.2 | **`limit<Rules>` live** — static discharge, runtime residue, subsumption; 0.9.0's rung refusal replaced. **PLANNED 2026-09-04 (`1.5.2.md`, execution-grade; L-0…L-30; §9's S-28…S-30 RATIFIED the same day as D-251, D-252 and the 1.5.4b row).** Six steps: **step 0 is DEF-14**, a soundness defect found by planning — the 1.5.0 encoder keeps a stable symbol for a local whose address a call takes, so a definition of another local in terms of it outlives the write and a division guard was DISCHARGED that the program then defeats (the verified build dies with SIGFPE where the plain build reaches `failsafe`; proven end to end on `8dbef43`); then the two checker refusals (a limited binding has no address, TYPE-063; a `limit` where no write point exists, TYPE-064) and `LimitViolated` (−4111) armed by REACH; the runtime check in EVERY build — one generated predicate function per `Rules`, the check AFTER every write over the binding's whole value (initialiser, every assignment to it or to any part of it, the callee's entry — sync and coroutine), the rung retired; the `limit` rows with the rule as a HYPOTHESIS on every later version (the first cross-kind discharge: a `div-zero` under a limited divisor, after a loop), `limit-subsume` rows at every direct call, elision into ONE `llvm.assume` over the rule's range clauses, both runners kind-aware; the caller-side bypass (a discharged `limit-subsume` row lets a direct call skip the callee's entry check — S-29); the docs and D-251/D-252. Unscheduled and raised as S-30: the QF_BV crossing, the twisted scaled-Int rows and the float tiers have no subcycle, so a `limit` over those subjects stays `unencoded` until one exists (recommended: 1.5.4b). | 1.5.1, 1.5.1b |
| 1.5.2b | **Derived comparisons over a generic-parameter field (D-253, S-24)** — the prelude implements `Eq`/`Ord`/`PartialOrd` for every scalar (generated from the width ladder as the `Hash` impls are), a derived impl whose subject is generic carries a synthesized bound, and a parameter field compares by the method form; frontend and prelude only, no snapshot refresh. Scheduled at ratification (2026-09-04); nothing is blocked by it today | 1.5.2 |
