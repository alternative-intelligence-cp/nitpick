...
| 1.5.2f | **A bare type parameter is move-only in the body that names it — PLANNED** (`1.5.2f.md`, 2026-09-05; S-40, the workbench's O-N19, ratified as **D-264**: `type_owns_for_move` answers true for a `T` and for `Self`, the lending `pick` and the derive generator ask the same question, seven sites respelled `move`; S-41, a borrowing `pick` binding form, is what it leaves open). Two steps under full harnesses | 1.5.2e |
