...
| 1.5.4c | **A module symbol means one thing (D-273, DEF-31) — NEXT, planned 2026-09-07** (`1.5.4c.md`, execution-grade): the qualified call `m.f(x)` is a direct call of the member (inline module, alias, nested path), `use m.*` and its selective forms bind a module symbol's public names, a private member is RESOLVE-004 from either spelling, an alias carries its file's scope, `std` is a name no program declares as a module; three steps under D-228. Scheduled ahead of 1.5.4b so the module system carries no known half-feature into the theories work | 1.5.4 |
| 1.5.4b | **The remaining theories (S-30, ratified 2026-09-04)** — `1.5.4b.md` is a PLANNING SKELETON (2026-09-07), not execution-grade: the settled scope, the questions and the measurements the resumed session completes before anything runs; 1.5.4c runs first — D-218.4/5's QF_BV crossing for bitwise operations, the scaled unbounded-Int encoding with ERR-sentinel rows for `tbb`/`tfp` (and `dim256` through it), and the two float tiers: none had a subcycle, and 1.5.8's `err-exit` rows presuppose the twisted encoding. Until it lands, a `limit` over such a subject and every division or overflow in those families is `unencoded` or absent (guard retained — safe, unproven). Planned when 1.5.4 closes, so the path-condition machinery exists first | 1.5.4 |
