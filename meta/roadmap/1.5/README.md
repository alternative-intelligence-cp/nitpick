...
| 1.5.2b | **Derived impls over generic subjects — PLANNED** (`1.5.2b.md`, 2026-09-05, execution-grade; D-253, S-24; its §9 ratified the same day as D-256…D-259, nothing pending on the user). The prelude implements `Eq`/`Ord`/`PartialOrd` for every scalar as a GENERATED region (the flags region's mechanism; the hand-listed `Hash` impls stop at 64 bits, which is why), a derived impl over a generic subject carries the derived trait as a bound on exactly the parameters its body reaches, and every member is reached through the trait being derived — a scalar through the prelude's impl, a named type or parameter through its own. **Planning found four defects, fixed in it**: DEF-15 a family impl's bound is declared and never enforced (`impl:<T: Ord>:Box<T>:Ord` admitted for a `Point` without `Ord`; `llc` refuses the IR), DEF-16 the no-bound operator story admits programs the emitter cannot lower (EMIT-002 at `Box<Point>`), DEF-17 the D-250 class for `Hash`/`Clone`/`ToString`/`Debug` (refusals inside `<derived-1>`), DEF-18 a derived `Clone` over a generic subject aliases an owner (a double free the allocator's instrument caught). Plus OPEN_DECISIONS §6's `DECL_THREAD` bit and the re-homing of every `<derived-N>` diagnostic to the derive. Six steps; frontend and prelude only, no snapshot refresh | 1.5.2 |
