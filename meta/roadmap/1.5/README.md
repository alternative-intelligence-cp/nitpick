...
> Detailed **map**, and deliberately short — most of 1.5's content is *gates that
...
## The gate that must close before 1.4 exits (C-19)
...
- that path must be scheduled *now* (a 1.4-or-earlier subcycle), not at 1.5.

So C-19 is promoted from the carried "confirm with AbsInt" note to a **numbered gate
answered before 1.4 exits**, with the full question list (below) written down. This
is the single most important item in the post-0.8 plan's tail, because it is the one
whose late discovery is unrecoverable.

## The AbsInt question list (settle during 1.4, at the latest)
...
| # | Topic | Gated on |
|---|---|---|
| 1.5.0 | **The confirmed input path** — whatever C-19 settled (a C-emitter if C-only, or the direct path if not); proven on a small program end-to-end into Astrée before the clock starts | C-19 |
| 1.5.1 | **Entry points, stubs, concurrency mapping** — the analysis harness Astrée needs, per the question list | C-19 |
| 1.5.2 | **The dry run** — a full pass on a representative program *before* the 30-day clock, so the real run finds analysis results, not setup errors | 1.5.0, 1.5.1 |

## Watch for

- **The clock starts once, and does not stop.** Every setup question answered on the
  30-day clock is a day not spent on the analysis that clock is for. 1.5.2's dry run
  is the whole risk-reduction of the cycle — treat a failed dry run as a schedule
  input, not a surprise.
- **The switch (`meta/SWITCH.md`) waits on 1.5** and inherits three audit
  ...
  These are folded into `OPEN_DECISIONS` and should be swept during 1.4/1.5 while both
  doc sets are still live — not after the archive.
- **Nothing about the switch happens until 1.5 is finished.** It is written down
