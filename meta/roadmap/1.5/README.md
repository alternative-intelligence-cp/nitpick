...
| 1.5.2d | **The fixed per-program cost — PLANNED** (`1.5.2d.md`, 2026-09-05, execution-grade; S-38, the library workbench's measurement at the 1.5.2c close, ratified the same day as **D-262**). Step 0 measured first, as the recommendation said: the frontend holds 88% of a floor-only program's 0.82 s, and a profile puts that in three scaling defects — the bindings analysis sizing its state by the whole program's statements for every function (57%), the type table and the string interner deduplicating by linear scan. Step 1: the frontend's cost bounded by the program (per-function slot ordinals at resolution; hash indexes over the two tables). Step 2: an unreferenced prelude item is not emitted, decided by a fixpoint over the emitted text, with a belt in both runners. Step 3: the docs and the workbench notice. Before 1.5.3 | 1.5.2c |
