...
| 1.5.5 | **The aliasing half of D-004 — PLANNED execution-grade 2026-09-11 on `cb8cbb0` (`1.5.5.md`; S-60…S-62 open for the user with recommendations; DEF-42, DEF-43 found by planning).** Measured: nothing decides aliasing today (two `$$m` of one element run; the spec's qualifier example does not parse — the operator form is the language's), a borrow assigned to an outer holder is accepted (DEF-42, rule 3 unchecked for local holders), and a write through a pointer to a `fixed` global is a SIGSEGV with no `failsafe` (DEF-43). The plan: `$$m` exclusive, `$$i` shared, `@` a plain address; lexical lifetimes (a call argument for its call, a pointer local's value for the holder's scope; NLL out); a claim only as a whole call argument or a pointer local's value (BORROW-014 elsewhere); a static overlap BORROW-013; a computed-index overlap a RUNTIME GUARD in every build (`BorrowOverlap`, −4116; a byte-range compare at the access) that the verified build elides when the `disjoint` row (kind 20) is discharged — D-068's shape, not the prototype's refuse-unless-verified model (S-61); a `fixed` binding has no address (TYPE-071, S-62). Four steps under D-228; step 0 (DEF-42) runs before ratification. The original row: VERIFICATION §2.1 presupposes the analysis — the conservative refusal Z3 then relaxes (the 0.5 analyses don't contain it; this cycle creates the error it suppresses) | 1.5.4e |
