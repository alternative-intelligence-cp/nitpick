...
| 1.5.4e | **The 1.5.4b close's three, landed (D-283…D-285; ratified 2026-09-10) — `1.5.4e.md` is PLANNED EXECUTION-GRADE (2026-09-10, on `18b93e1`)**: `simd` integer lanes carry D-210 (the vector overflow intrinsics any-lane, `IntOverflow`; an integer `.sum()` traps per fold step; the compound spelling lowers through the vector path — DEF-39 found by planning), `npk_raise` as the program's entry to the one trap route (`?!` and `!!!`; DEF-40 found by planning: `!!!` bypassed the trap entry), the binding-lanes reading of D-282 confirmed; three steps under D-228; `npkrt.o` moves at the close and the notice says so | 1.5.4b |
