...
| 1.5.7 | **The G-5 schedule-exploration harness — DONE (2026-09-18; eight landings, steps 0–7, each a cumulative prefix under a full harness, D-228; seats s8 then s10) (planned 2026-09-17 by s7, measured with a throw-away prototype, approved by the user the same day: S-77…S-83 → D-297…D-303; `1.5.7.md`)** — the REAL floor transformed (a scheduling point before each of its 78 atomic step lines, each of its 63 `@npk_sys6(` calls routed to a hand-written IR shim — X-1: a transform, never a second floor), real clone'd threads under a baton, virtual futex/epoll/clock, PCT with ordered priority bands and a fairness bound, exact replay by seed, quiescence ORACLES that see a lost wakeup an exit code cannot (`LOST-WAKE`/`LOST-FUTEX-WAKE` are red, D-301), negative controls from the models, the spec's caller hypotheses EXECUTED at every call (D-302), `// explore: N` markers with a belt (D-299), 1,000 seeds per unit (D-300). **Step 0 LANDED 2026-09-18**: the one transformer (`npkg/explore.npk`, `tools/explored.npk`) and the totality belt in both runners, the prototype kept outside every gate (D-303). **Step 1 LANDED 2026-09-18**: the shim in hand-written IR (`runtime/explore/npkx.ll`), held to the C reference schedule hash for schedule hash on all 30 signal-free programs under load; X-13 (the address space is virtual: the floor's `mmap` trims had made a step count depend on an address, found by the port in both shims); the `explore` stage in both runners; the markers on all 44 `// stress:` programs and the marker belt. **Step 2 LANDED 2026-09-18**: virtual signals — the four trap-route programs join, 34 of 34 explorable programs agreeing with the reference hash for hash. **Step 3 LANDED 2026-09-18**: the quiescence oracles live (`LOST-WAKE`/`LOST-FUTEX-WAKE` red, D-301; the offsets held to the floor's types), the control mechanism with `store-release` and `no-rouse` (both found at seed 1; 680 clean runs, no false positive). **Step 4 LANDED 2026-09-18**: the nineteen model controls walked — eleven `.ctl`s, five not floor bugs (the stamp-as-deadline, the handle discipline), one not observable, two not explorable; the grammar grew several pairs, `wrong-exit`/`exit N`, the lateness verdict `late N` (X-17) and DIRECTED controls (`preempt-at:`, X-15); three shim defects found by the walk and fixed in both shims (X-14 an expired deadline slept, X-16 the fairness bound resonated, the measuring run's line), 38 of 38 explorable programs agreeing with the reference; four new explored programs; and the step's own harness found the explorer's first FLOOR defect, **DEF-57** — an executor that saw the frozen flag trapped `Unreachable` itself and could win the failsafe holder in the window before the real trapper's claim, so `failsafe` ran with the wrong error (seed 371 of `trap_one_failsafe`) — fixed in the landing (a watcher parks, the holder keeps exit 70) with the `trap-route` model given the error code it lacked. **Step 5 LANDED 2026-09-18 (D-302)**: one generated entry checker per section with caller clauses (`npkg/explore_req.npk`; 31 checkers, 236 of 240 hypotheses executed at every call of every explored schedule, 4 listed by name in the stage's output and TCB.md §4d's new column), the verdict `ASSUMPTION`, spec controls (X-18) with `unconditional-apartness.ctl` finding 1.5.6's false clause at `drop_string`'s 27th step, and X-19 — a thread's end settled before anyone else steps, a real race of both shims the sweep found. **Step 6 LANDED 2026-09-18**: the program's own steps (X-20) — every explored unit's emitted IR through the same transformer's program mode (points before its `atomic<T>`/`atomic_from_ptr` steps, its `sys` calls routed, sites from 1,000,000) under the counting belt in both runners; `atomic_threads.npk` (two threads, one counter through `wild` storage) and the PROGRAM control (X-21: `program-old:`/`program-new:` over the source) `atomic-lost-update`, found at seed 1 and blind without the program's points. **Step 7 LANDED 2026-09-18**: the docs and the close — VERIFICATION_REFERENCE §10 (the explorer, whole), TCB.md (the explorer among the standing instruments; §5's thirteenth and sixteenth acceptances narrowed; a seventeenth for what it does not cover: weak memory, real-child programs, the trampoline, schedules beyond the seeds, liveness), the landing notes of D-212 and D-298…D-303, E-3's instrument half closed. Before step 0, S-77's own landing (D-297, `fa7b5a9`). | G-5 ratified; 1.5.6c |
| 1.5.8 | **The runtime's uncontrolled stops, closed — PLANNED 2026-09-18** (`1.5.8.md`; planned execution-grade by `nitpick-compiler_s11` on `e3bf48c`, S-84…S-87 ratified the same day — "go with all four" — as **D-304…D-307**). *[This row read, until 2026-09-18: "Overflow obligations (G-1's static leg) + close-out", with a 2026-09-17 note that VERIFICATION_REFERENCE §7b assigns 1.5.8 five kinds — `overflow`, `bounds`, `cast-range`, `terminate`, `stack-depth` — and that `terminate` had no surface. Planning measured first, and found three of the five kinds standing on uncontrolled stops, so the work became four subcycles in the order the user ratified.]* This one: the four identities declared together (`CastRange` 4117, `StackExhausted` 4118, `DecreasesViolated` 4119, `MachineFault` 4120), every `failsafe` REACH checks naming the two universal ones, and one bridging snapshot refresh (step 0); **DEF-58** — a float's `=>!` cast to an integer was LLVM poison (a program exits 3 at `-O0` and 9 after `opt -O2`) — fixed by the `CastRange` guard, and **DEF-61** — float↔`int128` casts lowered to compiler-rt calls — by hand-lowered wide conversions (step 1); **DEF-59**/**DEF-60** — a stack overflow killed with no `failsafe`, and a large frame could jump a thread's one guard page — fixed by LLVM's split-stack prologue on every emitted function, every thread's stack the floor's, signal stacks, and `failsafe` on a stack of its own (step 2); the last net for every other hardware fault (D-307, step 3); the refresh that carries the stack check into the builder, the docs (step 4). Three instrument defects on the way (DEF-62…DEF-64). | 1.5.7 |
| 1.5.8b | **The guarded kinds' rows** — `overflow` (D-210 §4: "1.5 proves the traps away"; 2,250 of the 2,519 guard sites in the compiler's own emission at `e3bf48c`, in 790 functions — a long tail, so discharges must come from general facts), `bounds` (15 sites there), `cast-range` (step 1's guard); their elision through `llvm.assume`; the runners' kind tables; the compiler's own manifest re-recorded. Planned execution-grade at 1.5.8's close. | 1.5.8 |
| 1.5.8c | **`decreases` and `unbounded` (D-304)** — the surface on `while`/`when` and functions, the check in every build (`DecreasesViolated`), the `terminate` rows (loops and recursive calls) and the `stack-depth` rows (D-305 (7): reported, never eliding), the sweep of ~925 loops in the tree, then the refusal (TYPE-072; TYPE-073/074/075 as D-304 states them). Planned execution-grade at 1.5.8b's close. | 1.5.8b |
| 1.5.8d | **The cycle's close** — NIKOS's disposition executed per D-217-as-annotated; C-19 closed by D-233 (no external gate remains); the docs synced; the cycle to `done/1.5/`. | 1.5.8c |
