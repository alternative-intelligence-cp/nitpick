...
| 1.5.4b | **The remaining theories (S-30, ratified 2026-09-04)** — `1.5.4b.md` is PLANNED EXECUTION-GRADE (2026-09-10, on `12a6a78`): six steps — a shift's amount checked (`ShiftRange`, `shift-range`; the probe showed poison reaching LLVM), bitwise in Int forms with a bounded crossing (measured: the crossing is free at 64 bits, 18% of the budget per row at 256, the budget at 2048), the twisted kinds as scaled Int with ERR a value and `err-exit` rows live, floats in two tiers with the tier column fed by the encoder and tier 2's three soundness conditions, `simd` lanes, the docs — pending the ratification of S-52…S-57 (raised the same day; nothing runs until then) — D-218.4/5's QF_BV crossing for bitwise operations, the scaled unbounded-Int encoding with ERR-sentinel rows for `tbb`/`tfp` (and `dim256` through it), and the two float tiers: none had a subcycle, and 1.5.8's `err-exit` rows presuppose the twisted encoding. Until it lands, a `limit` over such a subject and every division or overflow in those families is `unencoded` or absent (guard retained — safe, unproven). Planned when 1.5.4 closes, so the path-condition machinery exists first | 1.5.4 |
