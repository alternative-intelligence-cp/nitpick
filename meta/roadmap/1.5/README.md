...
| 1.5.2i | **`string_concat` of two empties allocates nothing — PLANNED (2026-09-06)** (`1.5.2i.md`; DEF-25, the library workbench's report: `@npk_string_concat` allocated a real block for a length-0 result and returned it with cap 0, so its drop freed nothing — 16 bytes per empty call, the prelude's `string:Clone` of an empty string and the compiler's own `string_concat(x, "")` copy idiom included). The slice's empty branch in the concat, a cost unit holding the empty loop's peak to the one-byte loop's; two steps, each under a full harness on a cumulative prefix. |
