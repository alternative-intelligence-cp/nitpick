...
| 1.5.2e | **Two small defects from 1.5.2d's close — PLANNED** (`1.5.2e.md`, 2026-09-05; S-39 ratified as **D-263**: the prelude's `List<T>` stores through `alloc_managed`, the managed heap's untracked entry, prelude-only by TYPE-054, and D-151 keeps counting every `wild` block; DEF-22, the workbench's O-N18: `.len` on a fixed-size array lowers to its constant). Three steps, each under a full harness | 1.5.2d |
