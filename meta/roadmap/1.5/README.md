...
| 1.5.6c | **The floor spec's caller assumptions (leads E-1 and E-2; approved 2026-09-17, to run BEFORE 1.5.7's plan).** `(objects …)` asserts its ranges pairwise APART as a HYPOTHESIS, and nothing checks the callers no row covers (a boundary symbol, the emitter's generated calls). A first walk found TWO FALSE CLAUSES, neither a behavioural defect, both evidence that says nothing where it looks like it speaks: `npk_string_concat` assumes its two INPUT strings apart and `string_concat(s, s)` is a legal program, in the tree twice (with the hypothesis deleted all 12 rows still discharge — it was never needed); `npk_small_free` assumes the chunk, its list neighbours and the class's PARTIAL-LIST HEAD apart unconditionally, and in the allocator's most common state — a free into the chunk at the head of the partial list — two of them are the same 64 KiB, so its 7 discharged rows claim nothing for the ordinary LIFO free (made conditional on "the chunk was full", the same 7 + 6 verdicts row for row at the same cost). Plus a latent trap: a second section-level `(objects …)` clause is silently ignored. To plan execution-grade: the two clauses, a spec form for READ-ONLY inputs that may alias, the duplicate-clause refusal in both runners, every other section's apartness ARGUED in a comment (the run queue, the waiter lists, the arenas, the chunk lists — sound by construction, first pass), and E-2's sentence in TCB.md saying these are the callers' to keep and which callers no row covers | 1.5.6b |
