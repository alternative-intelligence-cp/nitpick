...
| 1.5.2c | **Generic enums, and a `pick` over an `Optional` refused by name — PLANNED** (`1.5.2c.md`, 2026-09-05, execution-grade; the two findings 1.5.2b recorded for the user, ratified as recommended the same day as **D-260** and **D-261**). Step 0: a `pick`'s selector may not be an `Optional` (TYPE-065; `??` and `== NIL` are the spellings). Step 1: a generic enum is a family as a generic struct is — every read of a variant's payload type binds the instance's arguments through `bind_instance`, the constructor's instance comes from the expected type or is inferred from the payload by the generic call's unifier, the emitter substitutes the enum type at construction and `pick`, and the derive test 1.5.2b dropped returns. Step 2: the docs. Before 1.5.3, which lowers contracts over enums | 1.5.2b |
