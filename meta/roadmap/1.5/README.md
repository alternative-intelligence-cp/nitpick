...
| 1.5.2h | **A lending `pick` binds views; the selector is frozen while a view is live — PLANNED (2026-09-06)** (`1.5.2h.md`; S-41 ratified as **D-266**; DEF-24, found by planning, fixed at its step 0). A binding of a lending `pick` is a read-only VIEW of the payload in place — typed as the payload, read by value, no copy at the bind, no address, no move, no drop — the consuming form keeps ownership transfer, the selector's root is frozen inside an arm that binds (TYPE-067), a view is refused every address a limited binding is refused, the IMPLICIT receiver address included (which TYPE-063 had missed: DEF-24, a limited struct written through a pointer receiver with no trap), and the derive generator's `Eq`/`Ord`/`PartialOrd`/`Clone` lift for `string` and `T` payloads. Three steps, each under a full harness on a cumulative prefix. |
