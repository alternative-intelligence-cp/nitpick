...
`k_semantics_proofs` **6/6 proof modules** with twenty-seven concrete `kprove` claims;
...
- **K proof runner integrated with CTest** — `run_k_proofs.sh` compiles the
    ...
    and pointer-path claim modules.
...
| K semantics | 🔧 Active | Executable formal semantics seed — `kompile`/`krun` core oracle, `kprove` proof hook, CTest integration, 100/100 core tests, 6/6 proof modules |
...
- ✅ **Proof-oriented `kprove` hook** — Haskell-backend proof runner integrated
    ...
    and local pointer and pointer-path claim modules are passing
