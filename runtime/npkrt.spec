...
(shared
  ...
  (word %npk.exec 11 stated "epfd: -1 until the owner creates its set (the static initializer's -1, DEF-69, 1.5.8 step 3c -- 0 is a descriptor); created and read by the owner; CLOSED and set to -1 by the joiner once the kernel has cleared the tid word -- the owner's last act, after which it runs no user code -- and BEFORE the slot is retired, so the next thread reborn into the pool entry finds -1 (DEF-66, 1.5.8 step 3b); a stale waker never reads it")
  (word %npk.exec 12 atomic)               ; evfd: -1 until created (the static initializer's, DEF-69 -- 0 is a descriptor), a release store at creation, acquire loads by rousers, the owner's own reads monotonic; KEPT across rebirths of the pool entry (DEF-66), so a stale rouse lands on the eventfd it meant
