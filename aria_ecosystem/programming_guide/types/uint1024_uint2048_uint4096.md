...
fn merkle_root(leaves: [uint256]) -> uint256 {
    ...
    for i in 0..(leaves.length / 2) {
        ...
        let combined = sha256_combine(leaves[i * 2], leaves[i * 2 + 1]);
...
fn mod_exp_window(base: uint2048, exp: uint2048, mod: uint2048, window_size: uint32) -> uint2048 {
    ...
    for window in get_windows(bits, window_size) {
...
fn has_small_prime_divisor(n: uint2048) -> bool {
    ...
    for p in SMALL_PRIMES {  // [2, 3, 5, 7, 11, ..., 7919]
        if n % uint2048(p) == 0 && n != uint2048(p) {
            return true;
        }
    }
    return false;
...
fn sum_uint64_array(arr: [uint64; 1000000]) -> uint64 {
    ...
    for val in arr {
        sum += val;  // Excellent cache locality! Each cache line has 8 values
    }
    return sum;
}
...
fn sum_uint4096_array(arr: [uint4096; 10000]) -> uint4096 {
    ...
    for val in arr {
        sum += val;  // TERRIBLE cache locality! Each value spans 8 cache lines!
    }
    return sum;
}
...
fn encrypt_many(messages: [uint2048]) -> [uint2048] {
    ...
    for msg in messages {
        ...
        results.push(mod_exp(msg, 65537, n));
    }
    return results;
}
...
fn encrypt_many_fast(messages: [uint2048], n: uint2048) -> [uint2048] {
    let results = [];
    for msg in messages {
        results.push(mod_exp(msg, 65537, n));  // Reuse precomputed n
    }
    return results;
}
```
