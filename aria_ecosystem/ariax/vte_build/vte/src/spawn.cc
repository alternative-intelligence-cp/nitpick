                   ...
                   size_t workbufsize) noexcept
{
        ...
        /* AriaX: Open /dev/null for FDs 3-5 (stddbg, stddati, stddato)
         * These will be placeholders until proper debug/data streams are implemented.
         */
        auto devnull_fd = open("/dev/null", O_RDWR | O_CLOEXEC);
        if (devnull_fd == -1)
                return ExecError::DUP2; /* Reuse DUP2 error for /dev/null failure */
        
        m_fd_map[3].first = devnull_fd;
        m_fd_map[4].first = devnull_fd;
        m_fd_map[5].first = devnull_fd;
