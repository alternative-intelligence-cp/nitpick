...
def add_type_suffixes(content: str) -> tuple[str, list[str]]:
    ...
    modified = re.sub(r"(int32:\w+\s*=\s*)0(\s*;)", r"\g<1>0i32\g<2>", modified)

    # Pattern 2: Integer 1 in assignments to int32
    modified = re.sub(r"(int32:\w+\s*=\s*)1(\s*;)", r"\g<1>1i32\g<2>", modified)

    # Pattern 3: Integer 0 in assignments to int64
    modified = re.sub(r"(int64:\w+\s*=\s*)0(\s*;)", r"\g<1>0i64\g<2>", modified)

    # Pattern 4: Integer 1 in assignments to int64
    modified = re.sub(r"(int64:\w+\s*=\s*)1(\s*;)", r"\g<1>1i64\g<2>", modified)
    ...
    for num in range(2, 20):  # Cover common small integers
        modified = re.sub(
            rf"(int64:\w+\s*=\s*){num}(\s*;)", rf"\g<1>{num}i64\g<2>", modified
        )

    # Pattern 6: Small integers in int32 assignments
    for num in range(2, 20):
        modified = re.sub(
            rf"(int32:\w+\s*=\s*){num}(\s*;)", rf"\g<1>{num}i32\g<2>", modified
        )

    # Pattern 7: Float 0.0 in flt64 assignments
    modified = re.sub(r"(flt64:\w+\s*=\s*)0\.0(\s*;)", r"\g<1>0.0f64\g<2>", modified)

    # Pattern 8: Float 1.0 in flt64 assignments
    modified = re.sub(r"(flt64:\w+\s*=\s*)1\.0(\s*;)", r"\g<1>1.0f64\g<2>", modified)

    # Pattern 9: Float 0.0 in flt32 assignments
    modified = re.sub(r"(flt32:\w+\s*=\s*)0\.0(\s*;)", r"\g<1>0.0f32\g<2>", modified)

    # Pattern 10: pass() return values with integer literals
    # Matches: pass(0); -> pass(0i32);
    modified = re.sub(r"pass\(0\)", r"pass(0i32)", modified)

    # Pattern 11: pass() with small positive integers
    for num in range(1, 10):
        modified = re.sub(rf"pass\({num}\)", rf"pass({num}i32)", modified)
    ...
    modified = re.sub(r"\[0\]", r"[0i64]", modified)

    for num in range(1, 10):
        modified = re.sub(rf"\[{num}\]", rf"[{num}i64]", modified)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Add type suffixes to Aria stdlib literals"
    )
    ...
    parser.add_argument(
        "--restore", action="store_true", help="Restore from .bak files"
    )
